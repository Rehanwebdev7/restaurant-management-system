...
import com.rms.modules.cashier.services.CashDeliveryZonesService;
...
@RestController
@RequestMapping("api/cashier/delivery_zones")
public class CashDeliveryZonesController {
    ...
    @Autowired
    private CashDeliveryZonesService cashDeliveryZonesService;
    
    @GetMapping("/distanceCalculator")
	public ResponseEntity<Object> distanceCalculate(@RequestHeader("access_token") String token,
			@RequestParam(defaultValue = "0", required = false) Double latitude,
			@RequestParam(defaultValue = "10", required = false) Double longitude) {
		try {
			Map<String, Object> result = cashDeliveryZonesService.getDistance(latitude, longitude, token);
    ...
    @PostMapping("/add")
    public ResponseEntity<Object> addDeliveryZones(@RequestHeader("access_token") String token,@RequestBody DeliveryZonesEntity delivery_zonesEntity) {
        try {
            String result = deliveryZonesServiceIMP.addDeliveryZones(delivery_zonesEntity,token);
            return ApiResponse.responseBuilder(result, "SUCCESS", HttpStatus.CREATED, "DeliveryZones added successfully");
         }catch (SecurityException e) {
            return ApiResponse.responseBuilder(null, "FAILURE", HttpStatus.UNAUTHORIZED, e.getMessage());
        } catch (RuntimeException e) {
            return ApiResponse.responseBuilder(null, "FAILURE", HttpStatus.NOT_FOUND, e.getMessage());
    ...
    public ResponseEntity<Object> getById( @RequestHeader("access_token") String token,@PathVariable Long id) {
        try {
            ...
            return ApiResponse.responseBuilder(result, "SUCCESS", HttpStatus.OK, "DeliveryZones retrieved successfully");
         }catch (SecurityException e) {
            return ApiResponse.responseBuilder(null, "FAILURE", HttpStatus.UNAUTHORIZED, e.getMessage());
        } catch (RuntimeException e) {
            return ApiResponse.responseBuilder(null, "FAILURE", HttpStatus.NOT_FOUND, e.getMessage());
    ...
    //***** Api- Update Record  ***** 
    @PutMapping("/update")
    public ResponseEntity<Object> updateDeliveryZones( @RequestHeader("access_token") String token,@RequestBody DeliveryZonesEntity delivery_zonesEntity) {
        try {
    ...
    public ResponseEntity<Object> addMultiple(@RequestHeader("access_token") String token,@RequestBody List<DeliveryZonesEntity> list) {
        try {
    ...
    public ResponseEntity<Object> getDeliveryZonesByCreatedatRange(
            @RequestHeader("access_token") String token,
            @RequestParam LocalDate fromDate,
            @RequestParam LocalDate toDate) {
        try {
            List<DeliveryZonesEntity> result = deliveryZonesServiceIMP.getDeliveryZonesByCreatedatBetween(fromDate, toDate, token);
    ...
    public ResponseEntity<Object> getDeliveryZonesByCreatedat(
            @RequestHeader("access_token") String token,
    ...
    public ResponseEntity<Object> getDeliveryZonesByCreatedatRangeWithPagination(
        ...
        try {
            Map<String, Object> result = deliveryZonesServiceIMP.getDeliveryZonesByCreatedatBetweenPagination(fromDate, toDate, pageNumber, pageSize, token);
            return ApiResponse.responseBuilder(result, "SUCCESS", HttpStatus.OK, "DeliveryZones fetched successfully");
        } catch (SecurityException e) {
            return ApiResponse.responseBuilder(null, "FAILURE", HttpStatus.UNAUTHORIZED, e.getMessage());
        } catch (Exception e) {
