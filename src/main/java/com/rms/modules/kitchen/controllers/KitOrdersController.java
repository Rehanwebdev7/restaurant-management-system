...
import com.rms.modules.kitchen.services.KitOrdersService;
...
public class KitOrdersController {
	...
	@Autowired
	private KitOrdersService kitOrdersService;
	...
	public ResponseEntity<byte[]> downloadOrdersExcel(@RequestHeader("access_token") String token,
		...
		try {
			// 📦 KITCHEN BASED EXCEL EXPORT
			ByteArrayInputStream in = kitOrdersService.streamExcel(token, fromDate, toDate);

			byte[] bytes = in.readAllBytes();

			// 📄 File name
			String fileName = "Kitchen_Orders_" + fromDate + "_to_" + toDate + ".xlsx";
	...
	public ResponseEntity<Object> getMenuCategoriesWithFilters(@RequestHeader("access_token") String token,
		...
		try {
			...
			Map<String, Object> result = kitOrdersService.getKitchenOrdersWithFilters(fromDate, toDate, status, // ✅
																												// correct
					searchValue, // ✅ correct
	...
	public ResponseEntity<Object> getKitchenOrdersWithFilters(@RequestHeader("access_token") String token,
		...
		try {
			...
			Map<String, Object> result = kitOrdersService.getOrdersKitchenId(fromDate, toDate, status, searchValue,
					pageNumber, pageSize, token);
