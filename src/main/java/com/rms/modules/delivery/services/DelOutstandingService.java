...
public class DelOutstandingService implements OutstandingServiceIMP {

	private final OutstandingRepository outstandingrepository;
	private final UsersRepository usersrepository;

	public DelOutstandingService(OutstandingRepository outstandingrepository, UsersRepository usersrepository) {
	 ...
	 */
	public String outstandingTransaction(Map<String, Object> payload) throws Exception {
		return javaProcedures.outstandingTransactionProcedure(payload);
	}
	...
	public ByteArrayInputStream streamOutstandingLedgerExcel(LocalDate fromDate, LocalDate toDate, String token)
		...
		if (from != null && to != null) {
			outstandingList = outstandingrepository.findByUserId_IdAndDateBetween(deliveryUserId, from, to);
		} else {
			outstandingList = outstandingrepository.findByUserId_Id(deliveryUserId);
		}
	...
	public Map<String, Object> getOutstandingWithFilters(LocalDate fromDate, LocalDate toDate, Integer mode,
		...
		Specification<OutstandingEntity> spec = (root, query, cb) -> {
			...
			Join<OutstandingEntity, UsersEntity> userJoin = root.join("userId", JoinType.INNER);
	...
	public String addOutstanding(OutstandingEntity outstandingEntity, String token) throws Exception {
		...
		System.out.println("🚀 === ADD OUTSTANDING STARTED ===");

		// ================= AUTH =================
		System.out.println("🔐 Authorizing delivery user");
		Authorization.authorizeDelivery(token);
		System.out.println("✅ Authorization successful");

		OutstandingEntity newEntity = new OutstandingEntity();
		System.out.println("🆕 New OutstandingEntity instance created");

		// ================= COPY NON-FK FIELDS =================
		System.out.println("📋 Copying non-foreign fields using reflection");
		...
		if (outstandingEntity.getUserId() != null && outstandingEntity.getUserId().getId() != null) {
			...
			System.out.println("👤 UserId provided: " + outstandingEntity.getUserId().getId());
			UsersEntity user = fetchReferenceById(outstandingEntity.getUserId(), usersrepository, "Users not found");

			newEntity.setUserId(user);
			System.out.println("✅ User reference set successfully");

		} else {
			System.out.println("⚠️ UserId not provided in request");
		}

		// ================= SAVE =================
		System.out.println("💾 Saving OutstandingEntity");
		outstandingrepository.save(newEntity);
		System.out.println("✅ OutstandingEntity saved successfully");

		System.out.println("🏁 === ADD OUTSTANDING COMPLETED ===");
