...
public class DelUsersProfileService implements UsersProfileServiceIMP {
	...
	public DelUsersProfileService(UsersProfileRepository usersprofilerepository, PincodesRepository pincodesrepository,
	...
	/* ================= UPDATE LOCATION API ================= */
	public String updateDeliveryLocation(String token, Double latitude, Double longitude) throws Exception {

		// 🔐 Authorization & Decrypt
		Authorization.authorizeDelivery(token);
		tokenUtil.decryptAndStoreToken(token);

		Integer deliveryId = tokenUtil.getCurrentUserId();

		// ⚡ Redis Update
		cacheData.saveOrUpdateLocation(deliveryId, latitude, longitude);

		return "Location updated successfully";
	}

	/* ================= FETCH LOCATION API ================= */
	public Map<String, Object> getDeliveryLocation(Integer deliveryId) {

		Map<String, Object> data = cacheData.getLocation(deliveryId);

		if (data == null) {
			throw new RuntimeException("Delivery location not found");
		}
		return data;
	}
