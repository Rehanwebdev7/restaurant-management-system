...
import com.rms.modules.restaurant.services.RestUsersProfileService;
...
@RestController
@RequestMapping("api/restaurant/users_profile")
public class RestUsersProfileController {
	...
	@Autowired
	private RestUsersProfileService usersProfileService;
	...
	public ResponseEntity<Object> addOrUpdateBranchProfile(@RequestHeader("access_token") String token,
		...
		try {
			String result = usersProfileService.addOrUpdateBranchProfile(body, token);
	...
	public ResponseEntity<Object> getByIds(@RequestHeader("access_token") String token,
		...
		try {
			...
			List<UsersProfileEntity> result = usersProfileService.getBybranchId(branchId, token);
