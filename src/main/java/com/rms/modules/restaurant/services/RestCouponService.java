...
public class RestCouponService implements CouponServiceIMP {
	...
	public RestCouponService(CouponRepository couponrepository, UsersRepository usersrepository) {
	...
	public Map<String, Object> getCouponsWithFilters(String token, Integer branchId, // 🔥 Optional Branch Filter
			LocalDate fromDate, LocalDate toDate, Boolean isActive, // (Optional if needed future use)
		...
		Specification<CouponEntity> spec = (root, query, cb) -> {
			...
			// ================= RESTAURANT FILTER =================
			Join<CouponEntity, UsersEntity> restaurantJoin = root.join("restaurantId", JoinType.INNER);

			predicates.add(cb.equal(restaurantJoin.get("id"), currentUserId.longValue()));

			// ================= BRANCH FILTER (OPTIONAL) =================
			if (branchId != null) {

				Join<CouponEntity, UsersEntity> branchJoin = root.join("branchId", JoinType.INNER);

				predicates.add(cb.equal(branchJoin.get("id"), branchId.longValue()));
			}
			...
			if (searchValue != null && !searchValue.trim().isEmpty()) {
				...
				// 🔹 STRING FIELDS
				searchPredicates.add(cb.like(cb.lower(root.get("couponName")), pattern));
				searchPredicates.add(cb.like(cb.lower(root.get("couponCode")), pattern));
				...
				try {
					...
					searchPredicates.add(cb.equal(root.get("discountAmount"), amount));
				} catch (Exception ignored) {
				}

				// 🔹 BOOLEAN (global / firstOrder)
				if (searchValue.equalsIgnoreCase("true") || searchValue.equalsIgnoreCase("false")) {
					Boolean boolVal = Boolean.valueOf(searchValue);
					searchPredicates.add(cb.equal(root.get("global"), boolVal));
					searchPredicates.add(cb.equal(root.get("firstOrder"), boolVal));
				}
		...
		Page<CouponEntity> page = couponrepository.findAll(spec, pageable);
...
//    public String addCouponMenuItem(Map<String, Object> payload, String token) throws Exception {
//
//        Authorization.authorizeRestaurant(token);
//
...
//
//        // 🔥 Extract menuItems separately
//        List<Integer> menuItemIds = null;
//        if (payload.containsKey("menuItems")) {
//            menuItemIds = (List<Integer>) payload.get("menuItems");
//            payload.remove("menuItems"); // remove before converting
//        }
//
//        // 🔥 Convert remaining payload to CouponEntity
//        CouponEntity couponEntity = mapper.convertValue(payload, CouponEntity.class);
//
//        CouponEntity newEntity = new CouponEntity();
//
//        // Copy non-foreign fields (same as your logic)
...
//
//        // 🔥 Get inserted ID
//        Integer generatedCouponId = savedCoupon.getId();
//
//        // 🔥 Handle menuItems mapping only if global = false
//        Boolean isGlobal = couponEntity.getGlobal();
//
//        if (Boolean.FALSE.equals(isGlobal) && menuItemIds != null && !menuItemIds.isEmpty()) {
//
...
//                mapping.setCouponId(savedCoupon); // use newly inserted coupon
...
//    }
	@Transactional
	public String addCouponMenuItemMultipart(MultipartFile logo, String payloadJson, String token) throws Exception {
		...
		List<Integer> menuItemIds = null;
		if (payload.containsKey("menuItems")) {
			menuItemIds = mapper.convertValue(payload.get("menuItems"), new TypeReference<List<Integer>>() {
			});
		...
		if (couponrepository.existsByCouponCodeAndIsDeleteFalse(couponEntity.getCouponCode())) {

			throw new RuntimeException("Coupon code already exists");
		}

		UsersEntity currentRestaurant = usersrepository.findById(currentUserId)
				.orElseThrow(() -> new RuntimeException("Restaurant not found"));

		// 🔥 Branch Validation (IMPORTANT)
		if (couponEntity.getBranchId() == null || couponEntity.getBranchId().getId() == null) {
			throw new RuntimeException("Branch is required");
		}

		UsersEntity branch = usersrepository.findByIdAndParentId_Id(couponEntity.getBranchId().getId(), currentUserId)
				.orElseThrow(() -> new RuntimeException("Branch does not belong to this restaurant"));

		// 🔥 FirstOrder Validation - ek branch me sirf ek hi firstOrder coupon ho sakta
		// hai
		if (Boolean.TRUE.equals(couponEntity.getFirstOrder())) {
			boolean firstOrderExists = couponrepository
					.existsByBranchId_IdAndFirstOrderTrueAndIsDeleteFalse(branch.getId());
			if (firstOrderExists) {
		...
		newEntity.setRestaurantId(currentRestaurant);
		newEntity.setAddedById(currentRestaurant);
		...
		// 🔥 Mapping Logic SAME
		if (Boolean.FALSE.equals(newEntity.getGlobal()) && menuItemIds != null && !menuItemIds.isEmpty()) {
	...
	@Transactional
	public String updateCouponMultipart(MultipartFile logo, String payloadJson, String token) throws Exception {
		...
		if (couponEntity.getCouponCode() != null && !couponEntity.getCouponCode().isBlank()) {

			boolean exists = couponrepository.existsByCouponCodeAndIdNotAndIsDeleteFalse(couponEntity.getCouponCode(),
					couponEntity.getId());
		...
		UsersEntity restaurantFromToken = usersrepository.findById(currentUserId)
				.orElseThrow(() -> new RuntimeException("Restaurant not found"));

		if (existingCoupon.getRestaurantId() == null
				|| !existingCoupon.getRestaurantId().getId().equals(restaurantFromToken.getId())) {
		...
		if (Boolean.TRUE.equals(couponEntity.getFirstOrder())) {
			boolean firstOrderExists = couponrepository
					.existsByBranchId_IdAndFirstOrderTrueAndIsDeleteFalseAndIdNot(branch.getId(), couponEntity.getId());
			if (firstOrderExists) {
				throw new RuntimeException("This branch already has a first order coupon");
			}
		}
		...
		existingCoupon.setBranchId(branch);
		...
		// 🔥 NEW MAPPING LOGIC (DELETE + REINSERT)

		if (Boolean.FALSE.equals(existingCoupon.getGlobal()) && menuItemIds != null) {

			// 1️⃣ Delete all old mappings
			couponMappingRepository.deleteByCouponId_Id(existingCoupon.getId());

			// 2️⃣ Reinsert new mappings
			if (!menuItemIds.isEmpty()) {
				...
				for (Integer menuId : menuItemIds) {
					...
					CouponMappingEntity mapping = new CouponMappingEntity();
					mapping.setCouponId(existingCoupon);
					mapping.setMenuItemId(menuItem);
	...
	public String addCoupon(CouponEntity couponEntity, String token) throws Exception {
		...
		if (couponEntity.getBranchId() != null && couponEntity.getBranchId().getId() != null) {
			newEntity.setBranchId(fetchReferenceById(couponEntity.getBranchId(), usersrepository, "Users not found"));
		}

		// Handle added_by_id foreign key
		if (couponEntity.getAddedById() != null && couponEntity.getAddedById().getId() != null) {
			newEntity.setAddedById(fetchReferenceById(couponEntity.getAddedById(), usersrepository, "Users not found"));
		}
	...
	public String updateCoupon(CouponEntity couponEntity, String token) throws Exception {
		...
		if (couponEntity.getBranchId() != null && couponEntity.getBranchId().getId() != null) {
			existingEntity
					.setBranchId(fetchReferenceById(couponEntity.getBranchId(), usersrepository, "Users not found"));
		}

		// Handle added_by_id foreign key
		if (couponEntity.getAddedById() != null && couponEntity.getAddedById().getId() != null) {
			existingEntity
					.setAddedById(fetchReferenceById(couponEntity.getAddedById(), usersrepository, "Users not found"));
		}
	...
	public String addMultipleCoupon(List<CouponEntity> couponEntitys, String token) throws Exception {
		...
		for (CouponEntity entity : couponEntitys) {
			...
			if (entity.getBranchId() != null && entity.getBranchId().getId() != null) {
				newEntity.setBranchId(fetchReferenceById(entity.getBranchId(), usersrepository, "Users not found"));
			}

			// Handle added_by_id foreign key
			if (entity.getAddedById() != null && entity.getAddedById().getId() != null) {
				newEntity.setAddedById(fetchReferenceById(entity.getAddedById(), usersrepository, "Users not found"));
			}
	...
	@Override
	public List<CouponEntity> getCouponByValidityBetween(LocalDate fromDate, LocalDate toDate, String token)
			throws Exception {
		Authorization.authorizeRestaurant(token);
	...
	@Override
	public Map<String, Object> getCouponByValidityBetweenPagination(LocalDate fromDate, LocalDate toDate,
	...
	@Override
	public List<CouponEntity> getCouponByCreatedatBetween(LocalDate fromDate, LocalDate toDate, String token)
			throws Exception {
		Authorization.authorizeRestaurant(token);
	...
	@Override
	public Map<String, Object> getCouponByCreatedatBetweenPagination(LocalDate fromDate, LocalDate toDate,
			Integer pageNumber, Integer pageSize, String token) throws Exception {
		Authorization.authorizeRestaurant(token);
		Pageable pageable = PageRequest.of(pageNumber, pageSize, Sort.by(Sort.Direction.DESC, "id"));
		LocalDateTime fromDateTime = fromDate.atStartOfDay();
...
//                row.createCell(2).setCellValue(couponEntity.getApplicableOn() != null ? couponEntity.getApplicableOn() : "N/A");
				row.createCell(3)
						.setCellValue(couponEntity.getCouponCode() != null ? couponEntity.getCouponCode() : "N/A");
				row.createCell(4)
						.setCellValue(couponEntity.getCouponName() != null ? couponEntity.getCouponName() : "N/A");
				row.createCell(5)
						.setCellValue(couponEntity.getDescription() != null ? couponEntity.getDescription() : "N/A");
				row.createCell(6)
						.setCellValue(couponEntity.getDisplayOnScreen() != null && couponEntity.getDisplayOnScreen()
								? "Active"
								: "Inactive");
//                row.createCell(7).setCellValue(couponEntity.getForUser() != null ? couponEntity.getForUser() : "N/A");
				row.createCell(8).setCellValue(
						couponEntity.getIsDelete() != null && couponEntity.getIsDelete() ? "Active" : "Inactive");
...
//                row.createCell(13).setCellValue(couponEntity.getType() != null ? couponEntity.getType() : "N/A");
				...
				row.createCell(15).setCellValue(
						couponEntity.getBranchId() != null ? couponEntity.getBranchId().toString() : "N/A");
				row.createCell(16).setCellValue(
						couponEntity.getRestaurantId() != null ? couponEntity.getRestaurantId().toString() : "N/A");
				row.createCell(17).setCellValue(
						couponEntity.getAddedById() != null ? couponEntity.getAddedById().toString() : "N/A");
				row.createCell(18).setCellValue(
						couponEntity.getGlobal() != null && couponEntity.getGlobal() ? "Active" : "Inactive");
				row.createCell(19).setCellValue(
						couponEntity.getIsPercent() != null && couponEntity.getIsPercent() ? "Active" : "Inactive");
				row.createCell(20).setCellValue(couponEntity.getQuantity() != null ? couponEntity.getQuantity() : 0);
				row.createCell(21)
						.setCellValue(couponEntity.getUsageLimit() != null ? couponEntity.getUsageLimit() : 0);
