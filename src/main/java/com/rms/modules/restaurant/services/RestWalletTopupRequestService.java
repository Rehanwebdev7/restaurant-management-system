...
@Service
@Qualifier("restWalletTopupRequestService")
public class RestWalletTopupRequestService implements WalletTopupRequestServiceIMP {

	private final WalletTopupRequestRepository wallettopuprequestrepository;
	private final UsersRepository usersrepository;

	public RestWalletTopupRequestService(WalletTopupRequestRepository wallettopuprequestrepository,
	...
	public ByteArrayInputStream streamExcel(String token, LocalDate fromDate, LocalDate toDate) throws Exception {
		...
		try (XSSFWorkbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
			...
			header.createCell(5).setCellValue("UTR");
			header.createCell(6).setCellValue("Status");
			header.createCell(7).setCellValue("Approved By");
			header.createCell(8).setCellValue("Approved Date");
			...
			for (WalletTopupRequestEntity entity : list) {
				...
				row.createCell(0)
						.setCellValue(entity.getUserId().getBranchId().getName() != null
								? entity.getUserId().getBranchId().getName()
								: "");
	...
	public ByteArrayInputStream streamExcelForALLDownline(String token, LocalDate fromDate, LocalDate toDate)
			throws Exception {
		...
		// 📦 FETCH DATA (TOKEN USER + DATE FILTER)
		List<WalletTopupRequestEntity> list = wallettopuprequestrepository
				.findByUserId_BranchId_ParentId_idAndDateBetween(currentUserId.longValue(), fromDateTime, toDateTime);
...
//				.orElseThrow(() -> new RuntimeException("Branch user not found"));
		...
		Specification<WalletTopupRequestEntity> spec = (root, query, cb) -> {
			...
			// 🔹 JOIN delivery → branch
			Join<UsersEntity, UsersEntity> branchJoin = deliveryUserJoin.join("branchId", JoinType.INNER);
			Join<UsersEntity, UsersEntity> parentRest = branchJoin.join("parentId", JoinType.INNER);

			// 🔒 MANDATORY BRANCH FILTER
			predicates.add(cb.equal(parentRest.get("id"), branchUserId));
