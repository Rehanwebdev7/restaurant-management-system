...
@Service
@Qualifier("restMenuItemsService")
public class RestMenuItemsService implements MenuItemsServiceIMP {
    ...
    public RestMenuItemsService(MenuItemsRepository menuitemsrepository, AddonsRepository addonsrepository, MenuCategoryRepository menucategoryrepository, MenuSubcategoryRepository menusubcategoryrepository, RestaurantBranchRepository restaurantbranchrepository, UsersRepository usersrepository) {
    ...
    public List<MenuItemsEntity> getMenuItemsByBranch(
            String token,
            Long branchId
    ) throws Exception {
        ...
        UsersEntity branch = usersRepository
                .findByIdAndParentId_Id(branchId, currentUserId)
                .orElseThrow(() ->
                        new RuntimeException("Branch does not belong to this restaurant"));

        // ✅ Fetch Menu Items (No Pagination)
        return menuItemsRepository
                .findByBranchId_IdAndIsDeletedFalse(branch.getId());
    }
    ...
    public String updateMultipleMenuItems(
        ...
        UsersEntity restaurant =
                usersrepository.findById(currentRestaurantId.longValue())
        ...
        for (MenuItemsEntity entity : menuItemsEntities) {
            ...
            // 🚨 SECURITY CHECK (same restaurant only)
            if (!existingEntity.getRestaurantId().getId()
                    .equals(currentRestaurantId.longValue())) {
            ...
            if (entity.getMenuCategoryId() != null && entity.getMenuCategoryId().getId() != null) {
                existingEntity.setMenuCategoryId(
                        fetchReferenceById(entity.getMenuCategoryId(),
            ...
            if (entity.getMenuSubcategoryId() != null && entity.getMenuSubcategoryId().getId() != null) {
                existingEntity.setMenuSubcategoryId(
                        fetchReferenceById(entity.getMenuSubcategoryId(),
                                menusubcategoryrepository,
                                "Menu subcategory not found")
                );
            }

            // 🔥 BRANCH IS COMPULSORY
            existingEntity.setBranchId(
                    fetchReferenceById(entity.getBranchId(),
                            usersRepository,
                            "Branch not found")
            );

            // 🔥 RESTAURANT FROM TOKEN ONLY
    ...
    public Map<String, Object> getMenuItemsWithFilters(
        ...
        Specification<MenuItemsEntity> spec = (root, query, cb) -> {
            ...
            // ================= RESTAURANT FILTER (ONLY OWN) =================
            Join<MenuItemsEntity, UsersEntity> restaurantJoin = root.join("restaurantId", JoinType.INNER);
...
//         // Active filter (outside search block)
	     ...
	     // ================= IS ACTIVE FILTER =================
//	     // 🔥 APPLY ONLY WHEN CATEGORY FILTER IS NOT USED
...
//	     }
            ...
            if (searchValue != null && !searchValue.trim().isEmpty()) {
                ...
                // 🔹 BRANCH SEARCH
                Join<MenuItemsEntity, UsersEntity> branchJoin = root.join("branchId", JoinType.LEFT);
...
//                    "branchId, menuCategoryId, menuSubcategoryId are mandatory");
//        }
//
//        // ================= VALIDATE BRANCH =================
//     // ================= VALIDATE BRANCH =================
//        UsersEntity branch = usersRepository
//                .findById(inputEntity.getBranchId().getId())
//                .orElseThrow(() -> new RuntimeException("Branch not found"));
//
//        // 🔥 NULL + OWNERSHIP CHECK
//        if (branch.getParentId() == null) {
//            throw new IllegalArgumentException("Branch is not linked with any restaurant");
//        }
//
//        if (!branch.getParentId().getId().equals(restaurant.getId())) {
//            throw new IllegalArgumentException("Branch does not belong to this restaurant");
//        }
//
//
//        // ================= VALIDATE CATEGORY =================
//        MenuCategoryEntity category = menuCategoryRepository
//                .findById(inputEntity.getMenuCategoryId().getId())
...
//
//        // ================= SET VERIFIED DATA =================
//        entity.setRestaurantId(restaurant);   // 🔥 FROM TOKEN
...
//    }
    ...
    public String addMenuItemWithImage(
        ...
        System.out.println("========== ADD MENU ITEM API HIT ==========");
        ...
        System.out.println("Payload JSON received:");
        ...
        System.out.println("Restaurant ID from token: " + currentRestaurantId);

        UsersEntity restaurant = usersRepository.findById(currentRestaurantId.longValue())
                .orElseThrow(() -> new RuntimeException("Restaurant not found from token"));

        System.out.println("Restaurant entity fetched: " + restaurant.getId()
        ...
        System.out.println("Parsed MenuItemsEntity: " + inputEntity);
        ...
        if (branchId == null || categoryId == null || subCategoryId == null) {
            System.out.println("❌ MANDATORY IDS MISSING");
            throw new IllegalArgumentException(
                    "branchId, menuCategoryId, menuSubcategoryId are mandatory");
        }
        ...
        System.out.println("Branch fetched: " + branch.getId()
                + " | parentId=" + (branch.getParentId() != null ? branch.getParentId().getId() : null));

        // 🔥 OWNERSHIP CHECK
        if (branch.getParentId() == null
                || !branch.getParentId().getId().equals(restaurant.getId())) {
            throw new IllegalArgumentException("Branch does not belong to this restaurant");
        }

        MenuCategoryEntity category = menuCategoryRepository.findById(categoryId)
                .orElseThrow(() -> new RuntimeException("Menu category not found"));
        System.out.println("Menu category fetched: " + category.getId());

        MenuSubcategoryEntity subcategory = menuSubcategoryRepository.findById(subCategoryId)
                .orElseThrow(() -> new RuntimeException("Menu subcategory not found"));
        System.out.println("Menu subcategory fetched: " + subcategory.getId());

        AddonsEntity addons = null;
        if (addonsId != null) {
            addons = addonsRepository.findById(addonsId)
                    .orElseThrow(() -> new RuntimeException("Addons not found"));
            System.out.println("Addons fetched: " + addons.getId());
        }
        ...
        System.out.println("Saving MenuItemsEntity...");
        ...
        if (inlineAddonsNode != null && inlineAddonsNode.isArray() && inlineAddonsNode.size() > 0) {
            ...
            if (addonGroup == null) {
                ...
                addonGroup.setName(savedEntity.getName() + " Add-ons");
            ...
            for (JsonNode addonNode : inlineAddonsNode) {
                ...
                boolean isRequired = addonNode.has("isRequired") && addonNode.get("isRequired").asBoolean(false);
    ...
    public String updateMenuItemWithImage(
        ...
        if (!existingEntity.getRestaurantId().getId().equals(restaurant.getId())) {
            throw new SecurityException("You are not allowed to update this menu item");
        }
        ...
        MenuCategoryEntity category = null;
        if (categoryId != null) {
            category = menuCategoryRepository.findById(categoryId)
                    .orElseThrow(() -> new RuntimeException("Menu category not found"));
        }

        MenuSubcategoryEntity subcategory = null;
        if (subCategoryId != null) {
            subcategory = menuSubcategoryRepository.findById(subCategoryId)
                    .orElseThrow(() -> new RuntimeException("Menu subcategory not found"));
        }
        ...
        // ================= SET VERIFIED FKs =================
        existingEntity.setRestaurantId(restaurant); // 🔥 from token
...
//                .orElseThrow(() -> new RuntimeException("Branch not found"));
//
//        // 🔥 IMPORTANT CHECK (branch belongs to same restaurant)
////        if (branch.getRestaurantId() == null ||
////            !branch.getRestaurantId().getId().equals(restaurant.getId())) {
////            throw new SecurityException("Branch does not belong to logged-in restaurant");
////        }
//
//        // ================= CATEGORY =================
//        MenuCategoryEntity category = null;
...
//        // ================= SUBCATEGORY =================
//        MenuSubcategoryEntity subcategory = null;
//        if (inputEntity.getMenuSubcategoryId() != null && inputEntity.getMenuSubcategoryId().getId() != null) {
//            subcategory = menuSubcategoryRepository
//                    .findById(inputEntity.getMenuSubcategoryId().getId())
//                    .orElseThrow(() -> new RuntimeException("Menu subcategory not found"));
//        }
//
...
////        existingEntity.setRestaurantId(restaurant);
//        existingEntity.setBranchId(branch);
//        if (category != null) existingEntity.setMenuCategoryId(category);
//        if (subcategory != null) existingEntity.setMenuSubcategoryId(subcategory);
//        existingEntity.setAddonsId(addons);
//
//        menuItemsRepository.save(existingEntity);
//
...
//                row.createCell(22).setCellValue(menu_itemsEntity.getIsDeleted() != null ? menu_itemsEntity.getIsDeleted() : 0);
                row.createCell(22)
				   .setCellValue(
				       Boolean.TRUE.equals(menu_itemsEntity.getIsDeleted())
				   );
