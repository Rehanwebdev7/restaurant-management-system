...
public class RestMenuCategoryService implements MenuCategoryServiceIMP {
    ...
    public RestMenuCategoryService(MenuCategoryRepository menucategoryrepository, RestaurantBranchRepository restaurantbranchrepository, UsersRepository usersrepository) {
...
//        Integer restaurantIdFromToken = tokenUtil.getCurrentUserId();
        ...
        Specification<MenuCategoryEntity> spec = (root, query, cb) -> {
            ...
            Join<MenuCategoryEntity, UsersEntity> restaurantJoin = root.join("restaurantId", JoinType.INNER);
...
//                // 🔹 BRANCH SEARCH
//                Join<MenuCategoryEntity, UsersEntity> branchJoin = root.join("branchId", JoinType.LEFT);
//                searchPredicates.add(cb.like(cb.lower(branchJoin.get("branchName")), pattern));
        ...
        Page<MenuCategoryEntity> page = menuCategoryRepository.findAll(spec, pageable);
    ...
    public String addMenuCategoryWithIcon(
        ...
        // 🔓 DECRYPT TOKEN (AS YOU USE EVERYWHERE)
        tokenUtil.decryptAndStoreToken(token);
        Integer currentUserId = tokenUtil.getCurrentUserId();
    ...
    public String deleteMenuCategory(Long id, String token) throws Exception {
        ...
        // Soft-delete related menu items to preserve order history references.
        ...
        // Soft-delete related subcategories so the tree hides them without violating FKs.
        List<MenuSubcategoryEntity> subcategories = menuSubcategoryRepository.findByMenuCategoryId_Id(id);
        if (!subcategories.isEmpty()) {
            for (MenuSubcategoryEntity subcategory : subcategories) {
