...
public class RestDiningTablesService implements DiningTablesServiceIMP {
	...
	public RestDiningTablesService(DiningTablesRepository diningtablesrepository,
	...
	public Map<String, Object> getDiningTablesWithFilters(LocalDate fromDate, LocalDate toDate, String searchValue,
			Integer capacity, Integer status, Integer pageNumber, Integer pageSize, String token) throws Exception {
		...
		Specification<DiningTablesEntity> spec = (root, query, cb) -> {
			...
			predicates.add(cb.equal(root.get("restaurantId").get("id"), currentRestaurantId));
			...
			if (searchValue != null && !searchValue.trim().isEmpty()) {
				...
				// 🔹 TABLE NUMBER
				try {
					Integer tableNo = Integer.valueOf(searchValue);
				...
				// 🔹 BRANCH
				Join<DiningTablesEntity, UsersEntity> branchJoin = root.join("branchId", JoinType.LEFT);
				searchPredicates.add(cb.like(cb.lower(branchJoin.get("name")), pattern));

				// 🔹 SECTION
				Join<DiningTablesEntity, SectionEntity> sectionJoin = root.join("sectionId", JoinType.LEFT);
		...
		Page<DiningTablesEntity> page = diningTablesRepository.findAll(spec, pageable);
	...
	public String addDiningTables(DiningTablesEntity dining_tablesEntity, String token) throws Exception {
		...
		if (dining_tablesEntity.getBranchId() != null && dining_tablesEntity.getBranchId().getId() != null) {
			...
			newEntity.setBranchId(
					fetchReferenceById(dining_tablesEntity.getBranchId(), usersRepository, "Branch not found"));
		} else {
