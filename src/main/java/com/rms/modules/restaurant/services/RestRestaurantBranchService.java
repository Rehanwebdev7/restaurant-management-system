...
public class RestRestaurantBranchService implements RestaurantBranchServiceIMP {
    ...
    public RestRestaurantBranchService(RestaurantBranchRepository restaurantbranchrepository, PincodesRepository pincodesrepository, UsersRepository usersrepository) {
...
//    }
    ...
    public String addRestaurantBranch(
        ...
        System.out.println("🆔 Restaurant ID from Token: " + currentUserId);
        ...
        // ================= SUBSCRIPTION CHECK =================
        List<SubscriptionEntity> activeSubs = subscriptionRepository.findActiveSubscriptionsByUserId(currentUserId.longValue());
        if (activeSubs.isEmpty()) {
            throw new RuntimeException("No active subscription. Please subscribe to a plan to create branches.");
        }
        SubscriptionEntity sub = activeSubs.get(0);
        Integer maxBranch = sub.getPlan().getMaxBranch();
        if (maxBranch != null) {
            long currentCount = restaurantBranchRepository.countByRestaurantId_IdAndIsDeletedFalse(currentUserId.longValue());
            if (currentCount >= maxBranch) {
        ...
        // ================= RESTAURANT ID (ONLY FROM TOKEN) =================
        UsersEntity restaurantUser = usersrepository.findById(currentUserId.longValue())
                .orElseThrow(() -> new RuntimeException("Restaurant not found from token"));

        newEntity.setRestaurantId(restaurantUser);

        System.out.println("✅ Restaurant ID set from token");

        // ================= SAVE =================
        restaurantbranchrepository.save(newEntity);

        // ================= CLEAR TOKEN (NO finally) =================
