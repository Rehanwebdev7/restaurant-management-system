...
import com.rms.modules.admin.services.AdmUsersProfileService;
...
@RestController
@RequestMapping("api/superadmin/users_profile")
public class SuperadminUsersProfileController {

	@Autowired
	@Qualifier("admUsersProfileService")
	private UsersProfileServiceIMP usersProfileServiceIMP;

	@Autowired
	private AdmUsersProfileService admUsersProfileService;
	
	
	@PostMapping("/add-update")
	public ResponseEntity<Object> addOrUpdateBranchProfile(@RequestHeader("access_token") String token,
			@RequestBody Map<String, Object> body) {

		try {
			String result = admUsersProfileService.addOrUpdateBranchProfile(body, token);
	...
	public ResponseEntity<Object> getByIds(@RequestHeader("access_token") String token,
		...
		try {
			...
			List<UsersProfileEntity> result = admUsersProfileService.getBybranchId(branchId, token);
	...
	public ResponseEntity<Object> getByRestaurantId(@RequestHeader("access_token") String token,
			@RequestParam("id") Long restaurantId) {

		try {

			UsersProfileEntity profile = admUsersProfileService.getProfileByRestaurantId(restaurantId, token);

			return ApiResponse.responseBuilder(profile, "SUCCESS", HttpStatus.OK,
	...
	public ResponseEntity<Object> bulkUpdateUsersProfile(@RequestHeader("access_token") String token,
			@RequestBody List<UsersProfileEntity> list) {

		try {
			String result = admUsersProfileService.bulkUpsertUsersProfileByRestaurant(list, token);
...
//	public ResponseEntity<Object> bulkUpsertUsersProfile(@RequestHeader("access_token") String token,
//			@RequestPart(value = "logos", required = false) List<MultipartFile> logos,
//			@RequestPart("payload") String payloadJson) {
//
//		try {
//			String result = admUsersProfileService.bulkUpsertUsersProfileWithLogo(logos, payloadJson, token);
//
//			return ApiResponse.responseBuilder(result, "SUCCESS", HttpStatus.CREATED,
//					"Users profile bulk upsert completed successfully");
//
//		} catch (SecurityException e) {
//			return ApiResponse.responseBuilder(null, "FAILURE", HttpStatus.UNAUTHORIZED, e.getMessage());
//
...
//	}
	...
	public ResponseEntity<Object> addUsersProfile(@RequestHeader("access_token") String token,
			...
			@RequestPart(value = "fevicon", required = false) MultipartFile coverImage,
			@RequestPart("payload") String payloadJson) {

		try {
			String result = admUsersProfileService.addUsersProfileWithLogo(logo, coverImage, payloadJson, token);
