...
@Service
@Qualifier("custPaymentGatewayService")
public class CustPaymentGatewayService implements PaymentGatewayServiceIMP {

	private final PaymentGatewayRepository paymentgatewayrepository;
	private final UsersRepository usersrepository;

	public CustPaymentGatewayService(PaymentGatewayRepository paymentgatewayrepository,
	...
	public List<PaymentGatewayEntity> getActivePaymentGatewaysByRestaurant(String token, Long restaurantId)
			throws Exception {
		// 🔓 TOKEN → CUSTOMER
		...
		List<PaymentGatewayEntity> gateways =
		        paymentGatewayRepository.findByRestaurantId_IdAndStatusTrue(restaurantId);

		if (gateways.isEmpty()) {
		    throw new RuntimeException("No active payment gateways found");
		}

		// ================= FILTER COD BASED ON CUSTOMER =================
		Boolean allowCod = customer.getAllowCod();
		System.out.println("Customer allow COD : " + allowCod);

		if (Boolean.FALSE.equals(allowCod)) {
		    gateways = gateways.stream()
		            .filter(gateway ->
		                    gateway.getPaymentMethod() == null
		                            || !"COD".equalsIgnoreCase(gateway.getPaymentMethod()))
		            .toList();
		}

		// ================= REMOVE CREDENTIALS FROM RESPONSE =================
		for (PaymentGatewayEntity gateway : gateways) {
		    gateway.setCredentials(null);
		}

		return gateways;
	}

//		List<PaymentGatewayEntity> gateways = paymentGatewayRepository.findByRestaurantId_IdAndStatusTrue(restaurantId);
//
//		if (gateways.isEmpty()) {
//			throw new RuntimeException("No active payment gateways found");
//		}
//		// 🔥🔥 REMOVE CREDENTIALS FROM RESPONSE ONLY
//		for (PaymentGatewayEntity gateway : gateways) {
//			gateway.setCredentials(null);
//		}
//
...
//	}
	...
	@Override
	public Map<String, Object> getAllPaymentGateway(Integer pageNumber, Integer pageSize, String token)
			throws Exception {
		Authorization.authorizeCustomer(token);
	...
	public String addPaymentGateway(PaymentGatewayEntity payment_gatewayEntity, String token) throws Exception {
		...
		if (payment_gatewayEntity.getRestaurantId() != null
				&& payment_gatewayEntity.getRestaurantId().getId() != null) {
	...
	@Override
	public String addMultiplePaymentGateway(List<PaymentGatewayEntity> payment_gatewayEntitys, String token)
			throws Exception {
		Authorization.authorizeCustomer(token);
	...
	public ByteArrayInputStream streamExcel(int pageNumber, int pageSize, String token) throws IOException {
		...
		try (XSSFWorkbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
			...
			for (PaymentGatewayEntity payment_gatewayEntity : page.getContent()) {
				...
				row.createCell(0)
						.setCellValue(payment_gatewayEntity.getId() != null ? payment_gatewayEntity.getId() : 0);
				row.createCell(1)
						.setCellValue(payment_gatewayEntity.getCredentials() != null
								? payment_gatewayEntity.getCredentials().toString()
								: "N/A");
				row.createCell(2).setCellValue(
						payment_gatewayEntity.getOnOf() != null ? payment_gatewayEntity.getOnOf() : "N/A");
				row.createCell(3)
						.setCellValue(payment_gatewayEntity.getStatus() != null && payment_gatewayEntity.getStatus()
								? "Active"
								: "Inactive");
				row.createCell(4).setCellValue(
						payment_gatewayEntity.getVendorname() != null ? payment_gatewayEntity.getVendorname() : "N/A");
				row.createCell(5)
						.setCellValue(payment_gatewayEntity.getRestaurantId() != null
								? payment_gatewayEntity.getRestaurantId().toString()
								: "N/A");
