...
public class CustMenuItemsService implements MenuItemsServiceIMP {
	...
	public CustMenuItemsService(MenuItemsRepository menuitemsrepository, AddonsRepository addonsrepository,
	...
	public Map<String, Object> getMenuItemsWithFilters_WithDietAndRecommend(
	        Long branchId,
	        Long categoryId,
	        Long subcategoryId,
	        String searchValue,
	    ...
	    System.out.println("====== getMenuItemsWithFilters() START ======");
	    System.out.println("Input branchId       : " + branchId);
	    System.out.println("Input categoryId     : " + categoryId);
	    System.out.println("Input subcategoryId  : " + subcategoryId);
	    System.out.println("Input searchValue    : " + searchValue);
	    System.out.println("Input pageNumber     : " + pageNumber);
	    System.out.println("Input pageSize       : " + pageSize);
	    System.out.println("Input dietaryType    : " + dietaryType);
	    ...
	    UsersEntity branchUser = usersRepository.findById(branchId).orElseThrow(() -> {
	        System.out.println("Invalid branchId provided: " + branchId);
	        return new RuntimeException("Invalid branchId");
	    });

	    System.out.println("Branch user found. BranchUser ID: " + branchUser.getId());

	    Specification<MenuItemsEntity> spec = (root, query, cb) -> {

	        System.out.println("---- Building JPA Specification ----");

	        // 🔥 VERY IMPORTANT
	        query.distinct(true);

	        List<Predicate> predicates = new ArrayList<>();
	        System.out.println("Predicate list initialized");

	        // ================= JOINS =================
	        System.out.println("Creating JOIN with branch");
	        Join<MenuItemsEntity, UsersEntity> branchJoin =
	                root.join("branchId", JoinType.INNER);

	        System.out.println("Creating LEFT JOIN with category");
	        Join<MenuItemsEntity, MenuCategoryEntity> categoryJoin =
	                root.join("menuCategoryId", JoinType.LEFT);

	        System.out.println("Creating LEFT JOIN with subcategory");
	        Join<MenuItemsEntity, MenuSubcategoryEntity> subcategoryJoin =
	                root.join("menuSubcategoryId", JoinType.LEFT);
	        ...
	        predicates.add(cb.or(cb.isTrue(root.get("isAvailable")), cb.isNull(root.get("isAvailable"))));
	        ...
	        // ================= RECOMMENDED =================
	        if (recommended != null) {
	            predicates.add(cb.equal(root.get("isRecommended"), recommended));
	        }

	        // ================= SEARCH =================
	        if (searchValue != null && !searchValue.trim().isEmpty()) {

	            System.out.println("Applying search filter ONLY on MenuItems.name");

	            String trimmedValue = searchValue.trim();
	            List<Predicate> searchPredicates = new ArrayList<>();

	            // ✅ STRING SEARCH (ONLY MENU ITEM NAME)
	            ...
	            // INTEGER SEARCH
	            try {
	                Integer num = Integer.valueOf(trimmedValue);
	                searchPredicates.add(cb.equal(root.get("priority"), num));
	                searchPredicates.add(cb.equal(root.get("preparationMinutes"), num));
	                searchPredicates.add(cb.equal(root.get("deliveryMinutes"), num));
	            } catch (Exception ignored) {
	            }

	            // DECIMAL SEARCH
	            try {
	                BigDecimal price = new BigDecimal(trimmedValue);
	                searchPredicates.add(cb.equal(root.get("price"), price));
	                searchPredicates.add(cb.equal(root.get("mrp"), price));
	                searchPredicates.add(cb.equal(root.get("costPrice"), price));
	            } catch (Exception ignored) {
	            }

	            // BOOLEAN SEARCH
	            if (trimmedValue.equalsIgnoreCase("true") || trimmedValue.equalsIgnoreCase("false")) {
	                Boolean boolVal = Boolean.valueOf(trimmedValue);
	                searchPredicates.add(cb.equal(root.get("isAvailable"), boolVal));
	                searchPredicates.add(cb.equal(root.get("availableOnline"), boolVal));
	            }

	            predicates.add(cb.or(searchPredicates.toArray(new Predicate[0])));
	        }

	        System.out.println("Final predicate count: " + predicates.size());
	    ...
	    System.out.println("====== getMenuItemsWithFilters() END ======");
	    return response;
	}

	
//	public Map<String, Object> getMenuItemsWithFilters_WithDietAndRecommend(Long branchId, Long categoryId,
//			Long subcategoryId, String searchValue, Integer pageNumber, Integer pageSize, Boolean dietaryType,
//			Boolean recommended, // ✅ NEW PARAMETER
//			String token) throws Exception {
//
//		System.out.println("====== getMenuItemsWithFilters() START ======");
//		System.out.println("Input branchId       : " + branchId);
...
//		System.out.println("Input pageSize       : " + pageSize);
//		System.out.println("Input dietaryType    : " + dietaryType);
//		System.out.println("Input recommended    : " + recommended); // ✅ LOG ADDED
//		System.out.println("Input token          : " + token);
//
//		// 🔐 AUTH
//		System.out.println("Authorizing customer...");
...
//			// ================= JOINS =================
//			System.out.println("Creating JOIN with branch");
//			Join<MenuItemsEntity, UsersEntity> branchJoin = root.join("branchId", JoinType.INNER);
//
//			System.out.println("Creating LEFT JOIN with category");
//			Join<MenuItemsEntity, MenuCategoryEntity> categoryJoin = root.join("menuCategoryId", JoinType.LEFT);
//
//			System.out.println("Creating LEFT JOIN with subcategory");
//			Join<MenuItemsEntity, MenuSubcategoryEntity> subcategoryJoin = root.join("menuSubcategoryId",
//					JoinType.LEFT);
//
//			// ================= DEFAULT FILTER =================
//			System.out.println("Applying default filters");
//
//			predicates.add(cb.equal(branchJoin.get("id"), branchUser.getId()));
//			System.out.println("✔ Branch filter applied");
//
//			predicates.add(cb.isFalse(root.get("isDeleted")));
//			System.out.println("✔ isDeleted = false applied");
//
//			predicates.add(cb.or(cb.isTrue(root.get("isActive")), cb.isNull(root.get("isActive"))));
//
//			predicates.add(cb.or(cb.isTrue(root.get("isAvailable")), cb.isNull(root.get("isAvailable"))));
//			System.out.println("✔ isActive / isAvailable filter applied");
//
//			// ================= CATEGORY =================
//			if (categoryId != null) {
//				System.out.println("Applying category filter: " + categoryId);
//				predicates.add(cb.equal(categoryJoin.get("id"), categoryId));
//			} else {
//				System.out.println("Category filter skipped");
//			}
//
//			// ================= SUB CATEGORY =================
//			if (subcategoryId != null) {
//				System.out.println("Applying subcategory filter: " + subcategoryId);
//				predicates.add(cb.equal(subcategoryJoin.get("id"), subcategoryId));
//			} else {
//				System.out.println("Subcategory filter skipped");
//			}
//
//			// ================= DIETARY TYPE =================
//			if (dietaryType != null) {
//				System.out.println("Applying dietaryType filter: " + dietaryType);
//				predicates.add(cb.equal(root.get("dietaryType"), dietaryType));
//				System.out.println("✔ dietaryType filter applied");
//			} else {
//				System.out.println("DietaryType filter skipped");
//			}
//
//			// ================= RECOMMENDED (NEW COMPULSORY FILTER) =================
//			if (recommended != null) {
//				System.out.println("Applying recommended filter: " + recommended);
//				predicates.add(cb.equal(root.get("isRecommended"), recommended));
//				System.out.println("✔ recommended filter applied");
//			} else {
//				System.out.println("Recommended filter skipped");
//			}
//
//			// ================= SEARCH =================
//			if (searchValue != null && !searchValue.trim().isEmpty()) {
//
//				System.out.println("Applying search filter for value: " + searchValue);
//
...
////				searchPredicates.add(cb.like(cb.lower(subcategoryJoin.get("name")), pattern));
//				System.out.println("✔ Text search predicates added");
//
//				// INTEGER
//				try {
//					Integer num = Integer.valueOf(searchValue);
//					searchPredicates.add(cb.equal(root.get("priority"), num));
//					searchPredicates.add(cb.equal(root.get("preparationMinutes"), num));
//					searchPredicates.add(cb.equal(root.get("deliveryMinutes"), num));
//					System.out.println("✔ Integer search predicates added");
//				} catch (Exception e) {
//					System.out.println("Not an integer search value");
//				}
//
//				// DECIMAL
//				try {
...
//					searchPredicates.add(cb.equal(root.get("mrp"), price));
//					searchPredicates.add(cb.equal(root.get("costPrice"), price));
//					System.out.println("✔ Decimal search predicates added");
//				} catch (Exception e) {
//					System.out.println("Not a decimal search value");
//				}
//
//				// BOOLEAN
//				if (searchValue.equalsIgnoreCase("true") || searchValue.equalsIgnoreCase("false")) {
//					Boolean b = Boolean.valueOf(searchValue);
//					searchPredicates.add(cb.equal(root.get("isAvailable"), b));
//					searchPredicates.add(cb.equal(root.get("availableOnline"), b));
////					searchPredicates.add(cb.equal(root.get("isRecommended"), b));
//					System.out.println("✔ Boolean search predicates added");
//				}
//
//				predicates.add(cb.or(searchPredicates.toArray(new Predicate[0])));
//				System.out.println("✔ Search OR predicate applied");
//			} else {
//				System.out.println("Search filter skipped");
//			}
//
//			System.out.println("Final predicate count: " + predicates.size());
//			return cb.and(predicates.toArray(new Predicate[0]));
//		};
//
//		System.out.println("Creating pageable object");
...
//
//		System.out.println("Executing DB query with specification & pagination");
//		Page<MenuItemsEntity> page = menuitemsrepository.findAll(spec, pageable);
//
//		System.out.println("Query executed");
//		System.out.println("Total records found: " + page.getTotalElements());
//
//		Map<String, Object> response = new LinkedHashMap<>();
//		response.put("totalRecords", page.getTotalElements());
//		response.put("pageSize", page.getSize());
...
//		return response;
//	}
//============== Without recommend parameter ====================
//	public Map<String, Object> getMenuItemsWithFilters_WithDiet(Long branchId, Long categoryId, Long subcategoryId,
//			String searchValue, Integer pageNumber, Integer pageSize, Boolean dietaryType, String token)
//			throws Exception {
//
//		System.out.println("====== getMenuItemsWithFilters() START ======");
//		System.out.println("Input branchId       : " + branchId);
...
//			predicates.add(cb.or(cb.isTrue(root.get("isActive")), cb.isNull(root.get("isActive"))));
//			predicates.add(cb.or(cb.isTrue(root.get("isAvailable")), cb.isNull(root.get("isAvailable"))));
//			System.out.println("✔ isActive true OR null applied");
//
//			// ================= CATEGORY =================
//			if (categoryId != null) {
//				System.out.println("Applying category filter: " + categoryId);
...
//	}
	...
	public Map<String, Object> getMenuItemsWithFilters(Long branchId, Long categoryId, Long subcategoryId,
