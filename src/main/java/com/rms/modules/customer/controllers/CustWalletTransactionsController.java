...
import com.rms.modules.customer.dto.FundTransferRequest;
import com.rms.modules.customer.services.CustWalletTransactionsService;
...
@RestController
@RequestMapping("api/customer/wallet_transaction")
public class CustWalletTransactionsController {
    ...
    @Autowired
    private CustWalletTransactionsService custWalletTransactionsService;
    ...
    public ResponseEntity<Object> transferFunds(@RequestHeader("access_token") String token,
            @RequestBody FundTransferRequest request) {
        try {
            Map<String, Object> result = custWalletTransactionsService.transferFunds(
                    request.getBankId(), request.getAmount(), request.getTransferMode(), token);
            return ApiResponse.responseBuilder(result, "SUCCESS", HttpStatus.OK,
                    "Fund transfer request submitted successfully");
        } catch (SecurityException e) {
            return ApiResponse.responseBuilder(null, "FAILURE", HttpStatus.UNAUTHORIZED, e.getMessage());
        } catch (RuntimeException e) {
