...
import com.rms.modules.customer.services.CustPaymentGatewayService;
...
@RestController
@RequestMapping("api/customer/payment_gateway")
public class CustPaymentGatewayController {
	...
	@Autowired
	private CustPaymentGatewayService custPaymentGatewayService;;

	@GetMapping("/restaurantId")
	public ResponseEntity<Object> getActivePaymentGateways(@RequestHeader("access_token") String token,@RequestParam Long restaurantId) {
		try {

			List<PaymentGatewayEntity> result = custPaymentGatewayService
					.getActivePaymentGatewaysByRestaurant(token,restaurantId);

			return ApiResponse.responseBuilder(result, "SUCCESS", HttpStatus.OK,
					"Active payment gateways fetched successfully");
	...
	@PostMapping("/add")
	public ResponseEntity<Object> addPaymentGateway(@RequestHeader("access_token") String token,
			@RequestBody PaymentGatewayEntity payment_gatewayEntity) {
		try {
			...
			return ApiResponse.responseBuilder(result, "SUCCESS", HttpStatus.CREATED,
					"PaymentGateway added successfully");
		} catch (SecurityException e) {
			return ApiResponse.responseBuilder(null, "FAILURE", HttpStatus.UNAUTHORIZED, e.getMessage());
		} catch (RuntimeException e) {
			return ApiResponse.responseBuilder(null, "FAILURE", HttpStatus.NOT_FOUND, e.getMessage());
	...
	public ResponseEntity<Object> getById(@RequestHeader("access_token") String token, @PathVariable Integer id) {
		try {
			...
			return ApiResponse.responseBuilder(result, "SUCCESS", HttpStatus.OK,
					"PaymentGateway retrieved successfully");
		} catch (SecurityException e) {
			return ApiResponse.responseBuilder(null, "FAILURE", HttpStatus.UNAUTHORIZED, e.getMessage());
		} catch (RuntimeException e) {
			return ApiResponse.responseBuilder(null, "FAILURE", HttpStatus.NOT_FOUND, e.getMessage());
	...
	// ***** Api- Update Record *****
	@PutMapping("/update")
	public ResponseEntity<Object> updatePaymentGateway(@RequestHeader("access_token") String token,
			@RequestBody PaymentGatewayEntity payment_gatewayEntity) {
		try {
	...
	public ResponseEntity<Object> addMultiple(@RequestHeader("access_token") String token,
			@RequestBody List<PaymentGatewayEntity> list) {
		try {
