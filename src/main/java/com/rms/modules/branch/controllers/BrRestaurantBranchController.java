...
public class BrRestaurantBranchController {
    ...
    @Autowired
    @Qualifier("brRestaurantBranchService")
    ...
    public ResponseEntity<Object> getBranchStatus(@RequestHeader("access_token") String token) {
        try {
            com.rms.configuration.Authorization.authorizeBranch(token);
            tokenUtil.decryptAndStoreToken(token);
            Long branchId = Long.valueOf(tokenUtil.getCurrentUserId());
            BranchStatusDTO result = branchStatusService.getBranchStatus(branchId);
    ...
    public ResponseEntity<Object> stopOrders(@RequestHeader("access_token") String token) {
        try {
            ...
            tokenUtil.decryptAndStoreToken(token);
            Long branchId = Long.valueOf(tokenUtil.getCurrentUserId());
            branchStatusService.stopOrders(branchId, "branch");
            BranchStatusDTO result = branchStatusService.getBranchStatus(branchId);
    ...
    public ResponseEntity<Object> resumeOrders(@RequestHeader("access_token") String token) {
        try {
            ...
            tokenUtil.decryptAndStoreToken(token);
            Long branchId = Long.valueOf(tokenUtil.getCurrentUserId());
            branchStatusService.resumeOrders(branchId);
            BranchStatusDTO result = branchStatusService.getBranchStatus(branchId);
