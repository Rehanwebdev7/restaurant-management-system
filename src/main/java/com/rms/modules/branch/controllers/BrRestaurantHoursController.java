...
public class BrRestaurantHoursController {
	...
	@GetMapping("/filter")
	public ResponseEntity<Object> getByBranchFromToken(@RequestHeader("access_token") String token) {

		try {

			// 🔥 branchId token se niklegi
			List<RestaurantHoursEntity> result = brRestaurantHoursService.getByBranchFromToken(token);

			return ApiResponse.responseBuilder(result, "SUCCESS", HttpStatus.OK,
					"Restaurant hours retrieved successfully");
	...
	public ResponseEntity<Object> upsertMultipleRestaurantHours(@RequestHeader("access_token") String token,
		...
		try {
			...
			String result = brRestaurantHoursService.upsertRestaurantHours(token, hoursList);
