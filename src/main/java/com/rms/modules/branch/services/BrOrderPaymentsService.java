...
@Service
@Qualifier("brOrderPaymentsService")
public class BrOrderPaymentsService implements OrderPaymentsServiceIMP {
    ...
    public BrOrderPaymentsService(OrderPaymentsRepository orderpaymentsrepository, OrdersRepository ordersrepository, RestaurantBranchRepository restaurantbranchrepository, UsersRepository usersrepository) {
    ...
    public Map<String, Object> getOrderPaymentsWithFilters(
            LocalDate fromDate,
            LocalDate toDate,
        ...
        Specification<OrderPaymentsEntity> spec = (root, query, cb) -> {
            ...
            Join<OrderPaymentsEntity, UsersEntity> branchJoin =
                    root.join("branchId", JoinType.INNER);

            predicates.add(
                    cb.equal(branchJoin.get("id"), currentBranchId.longValue())
            );

            // ================= RESTAURANT FILTER (via parentId) =================
            Join<UsersEntity, UsersEntity> restaurantJoin =
                    branchJoin.join("parentId", JoinType.INNER);

            predicates.add(cb.isNotNull(restaurantJoin.get("id")));
        ...
        Page<OrderPaymentsEntity> page =
                orderPaymentsRepository.findAll(spec, pageable);
    ...
    public OrderPaymentsEntity getOneOrderPayments(Long id, String token) throws Exception {
        Authorization.authorizeBranch(token);
        return orderpaymentsrepository.findById(id)
    ...
    public String addOrderPayments(OrderPaymentsEntity order_paymentsEntity, String token) throws Exception {
        ...
        if (order_paymentsEntity.getOnlineOrderId() != null && order_paymentsEntity.getOnlineOrderId().getId() != null) {
        ...
        if (order_paymentsEntity.getBranchId() != null && order_paymentsEntity.getBranchId().getId() != null) {
            newEntity.setBranchId(
                fetchReferenceById(order_paymentsEntity.getBranchId(), usersRepository, "Restaurant_branch not found")
            );
        }
    ...
    @Override
    public String addMultipleOrderPayments(List<OrderPaymentsEntity> order_paymentsEntitys, String token) throws Exception {
        Authorization.authorizeBranch(token);
    ...
    public ByteArrayInputStream streamExcel(int pageNumber, int pageSize, String token) throws IOException {
        ...
        try (XSSFWorkbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            for (OrderPaymentsEntity order_paymentsEntity : page.getContent()) {
                ...
                row.createCell(1).setCellValue(order_paymentsEntity.getOnlineOrderId() != null ? order_paymentsEntity.getOnlineOrderId().toString() : "N/A");
                row.createCell(2).setCellValue(order_paymentsEntity.getRestaurantId() != null ? order_paymentsEntity.getRestaurantId().toString() : "N/A");
                row.createCell(3).setCellValue(order_paymentsEntity.getBranchId() != null ? order_paymentsEntity.getBranchId().toString() : "N/A");
                row.createCell(4).setCellValue(order_paymentsEntity.getPaymentMethod() != null ? order_paymentsEntity.getPaymentMethod() : "N/A");
                row.createCell(5).setCellValue(order_paymentsEntity.getPaymentGateway() != null ? order_paymentsEntity.getPaymentGateway() : "N/A");
                row.createCell(6).setCellValue(order_paymentsEntity.getGatewayTransactionId() != null ? order_paymentsEntity.getGatewayTransactionId() : "N/A");
                row.createCell(7).setCellValue(order_paymentsEntity.getAmount() != null ? order_paymentsEntity.getAmount().doubleValue() : 0.0);
                row.createCell(8).setCellValue(order_paymentsEntity.getPaymentStatus() != null ? order_paymentsEntity.getPaymentStatus() : "N/A");
                ...
                row.createCell(10).setCellValue(order_paymentsEntity.getRefundAmount() != null ? order_paymentsEntity.getRefundAmount().doubleValue() : 0.0);
                row.createCell(11).setCellValue(order_paymentsEntity.getRefundReason() != null ? order_paymentsEntity.getRefundReason() : "N/A");
                ...
                row.createCell(13).setCellValue(order_paymentsEntity.getIsReconciled() != null && order_paymentsEntity.getIsReconciled() ? "Active" : "Inactive");
