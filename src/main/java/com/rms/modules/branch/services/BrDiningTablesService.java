...
public class BrDiningTablesService implements DiningTablesServiceIMP {
	...
	public BrDiningTablesService(DiningTablesRepository diningtablesrepository,
	...
	public Map<String, Object> getDiningTablesWithFilters(
	        LocalDate fromDate,
	        LocalDate toDate,
	        String searchValue,
	        Integer capacity,
	    ...
	    Specification<DiningTablesEntity> spec = (root, query, cb) -> {
	        ...
	        if (searchValue != null && !searchValue.trim().isEmpty()) {
	            ...
	            // 🔹 TABLE NUMBER
	            try {
	                String tableNo = String.valueOf(searchValue);
	                searchPredicates.add(cb.equal(root.get("tableNumber"), tableNo));
...
//	            // 🔹 QR CODE
//	            searchPredicates.add(cb.like(cb.lower(root.get("qrCode")), pattern));

	            // 🔹 SECTION
	            Join<DiningTablesEntity, SectionEntity> sectionJoin =
	                    root.join("sectionId", JoinType.LEFT);
	            searchPredicates.add(cb.like(cb.lower(sectionJoin.get("name")), pattern));
	    ...
	    Page<DiningTablesEntity> page =
	            diningTablesRepository.findAll(spec, pageable);
	...
	@Override
	public String addDiningTables(DiningTablesEntity diningTablesEntity, String token) throws Exception {
	    ...
	    // ================= TOKEN =================
	    tokenUtil.decryptAndStoreToken(token);
	    Integer branchUserId = tokenUtil.getCurrentUserId();

	    // ================= BRANCH =================
	    UsersEntity branchUser = usersRepository.findById(branchUserId.longValue())
	    ...
	    for (Field field : DiningTablesEntity.class.getDeclaredFields()) {
	        ...
	        Object value = field.get(diningTablesEntity);

	        // skip foreign keys
	        if (value != null
	                && !field.getName().equals("branchId")
	                && !field.getName().equals("restaurantId")
	                && !field.getName().equals("sectionId")) {
	    ...
	    // ================= SET FK FROM TOKEN =================
	    newEntity.setBranchId(branchUser);
	    newEntity.setRestaurantId(restaurantUser);

	    // ================= SECTION (FROM BODY) =================
	    if (diningTablesEntity.getSectionId() == null
	            || diningTablesEntity.getSectionId().getId() == null) {
	        throw new RuntimeException("Section is required");
	    }

	    SectionEntity section = sectionrepository.findById(
	            diningTablesEntity.getSectionId().getId())
	            .orElseThrow(() -> new RuntimeException("Section not found"));

	    newEntity.setSectionId(section);
