...
@Service
@Qualifier("brMenuCategoryService")
public class BrMenuCategoryService implements MenuCategoryServiceIMP {
    ...
    public BrMenuCategoryService(MenuCategoryRepository menucategoryrepository, RestaurantBranchRepository restaurantbranchrepository, UsersRepository usersrepository) {
    ...
    @Transactional
    public String addMenuCategoryWithIcon(
            MultipartFile icon,
            String payloadJson,
            String token
    ) throws Exception {
        ...
        MenuCategoryEntity inputEntity =
                mapper.readValue(payloadJson, MenuCategoryEntity.class);
        ...
        // ================= FORCE SET (TOKEN BASED) =================
        entity.setBranchId(branchUser);          // ✅ token se
        entity.setRestaurantId(restaurantUser); // ✅ parent se
        entity.setIsDeleted(false);

        // ================= SAVE =================
        MenuCategoryEntity savedEntity =
    ...
    @Transactional
    public String updateMenuCategoryWithIcon(
            MultipartFile icon,
            String payloadJson,
            String token
    ) throws Exception {
        ...
        // ================= FETCH EXISTING =================
        MenuCategoryEntity existingEntity = menuCategoryRepository
                .findById(inputEntity.getId())
                .orElseThrow(() -> new RuntimeException("Menu category not found"));

        // ================= SECURITY CHECK =================
        if (existingEntity.getBranchId() == null ||
            existingEntity.getRestaurantId() == null ||
            !existingEntity.getBranchId().getId().equals(branchUser.getId()) ||
            !existingEntity.getRestaurantId().getId().equals(restaurantUser.getId())) {

            throw new SecurityException("Unauthorized update attempt");
        }

        // ================= CAPTURE OLD ACTIVE STATE =================
        Boolean wasActive = existingEntity.getIsActive();
        ...
        Boolean newActive = inputEntity.getIsActive();

        existingEntity.setUpdatedAt(LocalDateTime.now());
        menuCategoryRepository.save(existingEntity);

        // ================= CASCADE DEACTIVATION =================
        // When category is deactivated, deactivate all subcategories and items under it
        if (newActive != null && !newActive && (wasActive == null || wasActive)) {
            List<MenuSubcategoryEntity> subcategories = menuSubcategoryRepository
                    .findByMenuCategoryId_IdAndIsDeletedFalse(existingEntity.getId());
            for (MenuSubcategoryEntity sub : subcategories) {
                sub.setIsActive(false);
                sub.setUpdatedAt(LocalDateTime.now());
            }
            menuSubcategoryRepository.saveAll(subcategories);

            List<MenuItemsEntity> items = menuItemsRepository
                    .findByMenuCategoryId_IdAndIsDeletedFalse(existingEntity.getId());
            for (MenuItemsEntity item : items) {
                item.setIsActive(false);
            }
            menuItemsRepository.saveAll(items);
        }
    ...
    public String deleteMenuCategory(Long id, String token) throws Exception {
        ...
        long itemCount = menuItemsRepository.countByMenuCategoryId_IdAndIsDeletedFalse(id);
        if (itemCount > 0) {
            throw new RuntimeException("Cannot delete category: " + itemCount
