...
public class AdmRestaurantBranchService implements RestaurantBranchServiceIMP {
	...
	public AdmRestaurantBranchService(RestaurantBranchRepository restaurantbranchrepository,
			PincodesRepository pincodesrepository, UsersRepository usersrepository) {
...
//	            }
//
//	            // 🔹 Restaurant ID (UsersEntity.id)
//	            if (searchValue.matches("\\d+")) {
//	                Long restaurantId = Long.valueOf(searchValue);
//	                searchPredicates.add(
//	                    cb.equal(root.get("restaurantId").get("id"), restaurantId)
//	                );
//	            }
//
...
//	}
	...
	@Override
	public Map<String, Object> getAllRestaurantBranch(Integer pageNumber, Integer pageSize, String token)
			throws Exception {
		Authorization.authorizeAdmin(token);
	...
	public String addRestaurantBranch(RestaurantBranchEntity restaurant_branchEntity, String token) throws Exception {
		...
		// ✅ GLOBAL UNIQUE CHECK
		boolean mobileExists = restaurantbranchrepository.existsByPhone(mobile);
		if (mobileExists) {
		...
		if (restaurant_branchEntity.getPincodeId() != null && restaurant_branchEntity.getPincodeId().getId() != null) {
			newEntity.setPincodeId(fetchReferenceById(restaurant_branchEntity.getPincodeId(), pincodesrepository,
					"Pincodes not found"));
		}

		// Handle restaurant_id foreign key
		if (restaurant_branchEntity.getRestaurantId() != null
				&& restaurant_branchEntity.getRestaurantId().getId() != null) {
	...
	@Override
	public String updateRestaurantBranch(RestaurantBranchEntity restaurant_branchEntity, String token)
			throws Exception {
		Authorization.authorizeAdmin(token);
		...
		// Handle pincode_id foreign key
		if (restaurant_branchEntity.getPincodeId() != null && restaurant_branchEntity.getPincodeId().getId() != null) {
			existingEntity.setPincodeId(fetchReferenceById(restaurant_branchEntity.getPincodeId(), pincodesrepository,
					"Pincodes not found"));
		}
	...
	public String deleteRestaurantBranch(Long id, String token) throws Exception {
		Authorization.authorizeAdmin(token);
		RestaurantBranchEntity record= restaurantbranchrepository.findById(id)
		.orElseThrow(() -> new RuntimeException("RestaurantBranch not found"));
...
//		restaurantbranchrepository.deleteById(id);
		...
		restaurantbranchrepository.save(record);
		return "Deleted Successfully";
	}

	@Override
	public String addMultipleRestaurantBranch(List<RestaurantBranchEntity> restaurant_branchEntitys, String token)
			throws Exception {
		Authorization.authorizeAdmin(token);
	...
	@Override
	public List<RestaurantBranchEntity> getRestaurantBranchByCreatedatBetween(LocalDate fromDate, LocalDate toDate,
			String token) throws Exception {
		Authorization.authorizeAdmin(token);
	...
	@Override
	public Map<String, Object> getRestaurantBranchByCreatedatBetweenPagination(LocalDate fromDate, LocalDate toDate,
	...
	@Override
	public List<RestaurantBranchEntity> getRestaurantBranchByCreatedat(LocalDate createdat, String token)
			throws Exception {
		Authorization.authorizeAdmin(token);
	...
	@Override
	public List<RestaurantBranchEntity> getRestaurantBranchByUpdatedatBetween(LocalDate fromDate, LocalDate toDate,
			String token) throws Exception {
		Authorization.authorizeAdmin(token);
		LocalDateTime fromDateTime = fromDate.atStartOfDay();
		LocalDateTime toDateTime = toDate.atTime(23, 59, 59);
	...
	@Override
	public Map<String, Object> getRestaurantBranchByUpdatedatBetweenPagination(LocalDate fromDate, LocalDate toDate,
			Integer pageNumber, Integer pageSize, String token) throws Exception {
		Authorization.authorizeAdmin(token);
		Pageable pageable = PageRequest.of(pageNumber, pageSize, Sort.by(Sort.Direction.DESC, "id"));
		LocalDateTime fromDateTime = fromDate.atStartOfDay();
	...
	@Override
	public List<RestaurantBranchEntity> getRestaurantBranchByUpdatedat(LocalDate updatedat, String token)
			throws Exception {
		Authorization.authorizeAdmin(token);
	...
	public ByteArrayInputStream streamExcel(int pageNumber, int pageSize, String token) throws IOException {
		...
		try (XSSFWorkbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
			...
			for (RestaurantBranchEntity restaurant_branchEntity : page.getContent()) {
				...
				row.createCell(0)
						.setCellValue(restaurant_branchEntity.getId() != null ? restaurant_branchEntity.getId() : 0);
				row.createCell(1).setCellValue(
						restaurant_branchEntity.getAddress() != null ? restaurant_branchEntity.getAddress() : "N/A");
				row.createCell(2)
						.setCellValue(restaurant_branchEntity.getBranchName() != null
								? restaurant_branchEntity.getBranchName()
								: "N/A");
				...
				row.createCell(4).setCellValue(
						restaurant_branchEntity.getEmail() != null ? restaurant_branchEntity.getEmail() : "N/A");
				row.createCell(5).setCellValue(
						restaurant_branchEntity.getIsActive() != null && restaurant_branchEntity.getIsActive()
								? "Active"
								: "Inactive");
				row.createCell(6).setCellValue(
						restaurant_branchEntity.getIsDeleted() != null && restaurant_branchEntity.getIsDeleted()
								? "Active"
								: "Inactive");
				row.createCell(7)
						.setCellValue(restaurant_branchEntity.getLatitude() != null
								? restaurant_branchEntity.getLatitude().doubleValue()
								: 0.0);
				row.createCell(8)
						.setCellValue(restaurant_branchEntity.getLongitude() != null
								? restaurant_branchEntity.getLongitude().doubleValue()
								: 0.0);
				row.createCell(9).setCellValue(
						restaurant_branchEntity.getPhone() != null ? restaurant_branchEntity.getPhone() : "N/A");
				row.createCell(10)
						.setCellValue(restaurant_branchEntity.getRestaurantId() != null
								? restaurant_branchEntity.getRestaurantId().toString()
								: "N/A");
				...
				row.createCell(12)
						.setCellValue(restaurant_branchEntity.getPincodeId() != null
								? restaurant_branchEntity.getPincodeId().toString()
								: "N/A");
