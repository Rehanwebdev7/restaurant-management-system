...
@Service
@Qualifier("admMenuCategoryService")
public class AdmMenuCategoryService implements MenuCategoryServiceIMP {
	...
	public AdmMenuCategoryService(MenuCategoryRepository menucategoryrepository,
	    ...
	    Specification<MenuCategoryEntity> spec = (root, query, cb) -> {
	     ...
	     // Active filter (outside search block)
	        ...
	        if (searchValue != null && !searchValue.trim().isEmpty()) {
	            ...
	            Join<MenuCategoryEntity, UsersEntity> restaurantJoin =
...
//	@Transactional
//	public String addMenuCategoryWithIcon(MultipartFile icon, String payloadJson, String token) throws Exception {
//
//		// ================= TOKEN =================
...
//		MenuCategoryEntity inputEntity = mapper.readValue(payloadJson, MenuCategoryEntity.class);
//
//		// ================= REQUIRED IDS =================
...
//
//			throw new IllegalArgumentException("restaurantId and branchId are mandatory");
//		}
//
//		// ================= VALIDATE IDS =================
...
//
//		RestaurantBranchEntity branch = restaurantBranchRepository.findById(inputEntity.getBranchId().getId())
...
//
//		MenuCategoryEntity savedEntity = menuCategoryRepository.save(entity);
//
//		// ================= ICON UPLOAD =================
//		if (icon != null && !icon.isEmpty()) {
//
//			String fileName = "menu_category_" + savedEntity.getId();
//			String iconUrl = googleDriveUtil.uploadFile(icon, fileName);
//
//			savedEntity.setIconUrl(iconUrl);
//			savedEntity.setUpdatedAt(LocalDateTime.now());
//
//			menuCategoryRepository.save(savedEntity);
//		}
//
//		return "Menu category added successfully";
//	}
	@Transactional
	public String addMenuCategoryWithIcon(MultipartFile icon, String payloadJson, String token) throws Exception {
		...
		// ================= VALIDATE RESTAURANT (FIX HERE) =================
		UsersEntity restaurant = usersRepository.findById(inputEntity.getRestaurantId().getId())
				.orElseThrow(() -> new RuntimeException("Restaurant not found"));

		// 🔥 IMPORTANT CHECK (ADMIN / CUSTOMER BLOCK)
		...
		if (icon != null && !icon.isEmpty()) {
			...
			String fileName = "menu_category_" + savedEntity.getId();
//			String fileName = UploadFolders.MENU_ITEMS + "menu_item_" + savedEntity.getId();

			// old: String iconUrl = googleDriveUtil.uploadFile(icon, fileName, "Menu_category");
			final Long _entityId = savedEntity.getId();
			String iconUrl = fileUploadService.uploadFile(icon, fileName, "Menu_category",
			    driveUrl -> menuCategoryRepository.updateDriveIconUrl(_entityId, driveUrl));
			savedEntity.setIconUrl(iconUrl);
			savedEntity.setUpdatedAt(LocalDateTime.now());
	...
	@Transactional
	public String updateMenuCategoryWithIcon(MultipartFile icon, String payloadJson, String token) throws Exception {
		...
		// ================= FETCH EXISTING =================
		MenuCategoryEntity existingEntity = menuCategoryRepository.findById(inputEntity.getId())
		...
		if (icon != null && !icon.isEmpty()) {
			...
			String fileName = "menu_category_" + existingEntity.getId();
	...
	public MenuCategoryEntity getOneMenuCategory(Long id, String token) throws Exception {
		Authorization.authorizeAdmin(token);
		return menucategoryrepository.findById(id).orElseThrow(() -> new RuntimeException("MenuCategory not found"));
	}
	...
	public String addMenuCategory(MenuCategoryEntity menu_categoryEntity, String token) throws Exception {
		...
		if (menu_categoryEntity.getBranchId() != null && menu_categoryEntity.getBranchId().getId() != null) {
			newEntity.setBranchId(fetchReferenceById(menu_categoryEntity.getBranchId(), usersRepository,
	...
	public String updateMenuCategory(MenuCategoryEntity menu_categoryEntity, String token) throws Exception {
		...
		// Handle branch_id foreign key
		if (menu_categoryEntity.getBranchId() != null && menu_categoryEntity.getBranchId().getId() != null) {
			existingEntity.setBranchId(fetchReferenceById(menu_categoryEntity.getBranchId(), usersRepository,
					"Restaurant_branch not found"));
		}
	...
	public String deleteMenuCategory(Long id, String token) throws Exception {
		...
		// Cascade delete: remove all menu items under this category first
		List<MenuItemsEntity> items = menuItemsRepository.findByMenuCategoryId_Id(id);
		if (!items.isEmpty()) {
			menuItemsRepository.deleteAll(items);
		}
		// Then remove all subcategories under this category
		List<MenuSubcategoryEntity> subcategories = menuSubcategoryRepository.findByMenuCategoryId_Id(id);
		if (!subcategories.isEmpty()) {
			menuSubcategoryRepository.deleteAll(subcategories);
		}
	...
	@Override
	public String addMultipleMenuCategory(List<MenuCategoryEntity> menu_categoryEntitys, String token)
			throws Exception {
		Authorization.authorizeAdmin(token);
	...
	@Override
	public List<MenuCategoryEntity> getMenuCategoryByCreatedatBetween(LocalDate fromDate, LocalDate toDate,
	...
	@Override
	public Map<String, Object> getMenuCategoryByCreatedatBetweenPagination(LocalDate fromDate, LocalDate toDate,
	...
	@Override
	public List<MenuCategoryEntity> getMenuCategoryByUpdatedatBetween(LocalDate fromDate, LocalDate toDate,
			String token) throws Exception {
		Authorization.authorizeAdmin(token);
		LocalDateTime fromDateTime = fromDate.atStartOfDay();
		LocalDateTime toDateTime = toDate.atTime(23, 59, 59);
	...
	@Override
	public Map<String, Object> getMenuCategoryByUpdatedatBetweenPagination(LocalDate fromDate, LocalDate toDate,
			Integer pageNumber, Integer pageSize, String token) throws Exception {
		Authorization.authorizeAdmin(token);
		Pageable pageable = PageRequest.of(pageNumber, pageSize, Sort.by(Sort.Direction.DESC, "id"));
		LocalDateTime fromDateTime = fromDate.atStartOfDay();
	...
	public ByteArrayInputStream streamExcel(int pageNumber, int pageSize, String token) throws IOException {
		...
		try (XSSFWorkbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
			...
			for (MenuCategoryEntity menu_categoryEntity : page.getContent()) {
				...
				row.createCell(1)
						.setCellValue(menu_categoryEntity.getRestaurantId() != null
								? menu_categoryEntity.getRestaurantId().toString()
								: "N/A");
				row.createCell(2).setCellValue(
						menu_categoryEntity.getBranchId() != null ? menu_categoryEntity.getBranchId().toString()
								: "N/A");
				row.createCell(3)
						.setCellValue(menu_categoryEntity.getName() != null ? menu_categoryEntity.getName() : "N/A");
				row.createCell(4).setCellValue(
						menu_categoryEntity.getDescription() != null ? menu_categoryEntity.getDescription() : "N/A");
				row.createCell(5).setCellValue(
						menu_categoryEntity.getPriority() != null ? menu_categoryEntity.getPriority() : 0);
				row.createCell(6)
						.setCellValue(menu_categoryEntity.getIsActive() != null && menu_categoryEntity.getIsActive()
								? "Active"
								: "Inactive");
				row.createCell(7)
						.setCellValue(menu_categoryEntity.getIsDeleted() != null && menu_categoryEntity.getIsDeleted()
								? "Active"
								: "Inactive");
				row.createCell(8).setCellValue(
						menu_categoryEntity.getIconUrl() != null ? menu_categoryEntity.getIconUrl() : "N/A");
				row.createCell(9)
						.setCellValue(menu_categoryEntity.getTaxPercentage() != null
								? menu_categoryEntity.getTaxPercentage().doubleValue()
								: 0.0);
