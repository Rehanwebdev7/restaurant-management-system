...
const Addons = () => {
  ...
  const fetchAddonsData = async () => {
    ...
    try {
      ...
      const result = await ApiGet('/api/admin/addons/filter', params);
  ...
  const fetchAddonItems = async (addonsId) => {
    try {
      const result = await ApiGet('/api/admin/addons_items/addonsId', { id: addonsId });
  ...
  const paginatedData = statusFilter
    ? apiData.filter(addon => {
        const active = addon.isActive === true || addon.isActive === 1;
  ...
  const handleEdit = async (addon) => {
    ...
    setFormData({
      name: addon.name || '',
      description: addon.description || '',
      restaurantId: addon.restaurantId?.id || '',
    ...
    setSelectedRestaurantName(addon.restaurantId?.name || '');
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const result = await ApiPut('/api/admin/addons/update_addonItem', payload);
  ...
  const handleModalSubmit = async (e) => {
    ...
    try {
      ...
      let result;
      if (modalMode === 'add') {
        result = await ApiPost('/api/admin/addons/add_addonItem', payload);
      } else {
        payload.id = selectedAddon.id;
        result = await ApiPut('/api/admin/addons/update_addonItem', payload);
      }
                      ...
                      <Select
                        ...
                        value={formData.branchId !== '' && formData.branchId != null ? {
                          ...
                          label: filteredBranches.find(b => b.id === Number(formData.branchId))?.name
                            ...
                            : branches.find(b => b.id === Number(formData.branchId))?.name
                              ? `${branches.find(b => b.id === Number(formData.branchId))?.name} (ID: ${formData.branchId})`
                              : selectedBranchName
