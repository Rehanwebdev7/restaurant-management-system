...
const PaymentGateway = () => {
  ...
  const fetchPaymentGatewayData = async () => {
    ...
    try {
      ...
      const result = await ApiGet('/api/admin/payment_gateway/getAll', params);
  ...
  const exportToExcel = () => {
    const exportData = apiData.map(gateway => ({
      ...
      'Key ID': gateway.credentials?.key_id || '',
  ...
  const handleEdit = (gateway) => {
    ...
    setFormData({
      ...
      restaurantId: gateway.restaurantId?.id || ''
    });
    setFormErrors({});
    // Set selected restaurant for react-select
    if (gateway.restaurantId) {
      setSelectedRestaurant({
        value: gateway.restaurantId.id,
        label: gateway.restaurantId.name
      });
    } else {
      setSelectedRestaurant(null);
    }
  ...
  const handleDelete = async (gateway) => {
    ...
    try {
      const result = await ApiDelete(`/api/admin/payment_gateway/${gateway.id}`);
  ...
  const handleStatusToggle = async (gateway) => {
    ...
    try {
      const payload = {
        ...
        restaurantId: gateway.restaurantId
      };

      const result = await ApiPut('/api/admin/payment_gateway/update', payload);
  ...
  const handleModalSubmit = async (e) => {
    ...
    try {
      ...
      let result;
      if (modalMode === 'add') {
        result = await ApiPost('/api/admin/payment_gateway/add', payload);
      } else {
        result = await ApiPut('/api/admin/payment_gateway/update', payload);
      }
  ...
  const getOnOfBadge = (onOf) => {
    if (onOf === 'ON') {
      return <Badge bg="success">ON</Badge>;
    } else {
            ...
            {loading ? (
              ...
              paginatedData.map((gateway) => (
                <tr key={gateway.id}>
                  ...
                  <td>{gateway.restaurantId?.name || restaurants.find(r => r.id === gateway.restaurantId?.id)?.name || 'N/A'}</td>
                ...
                <Form.Group className="mb-3">
                  ...
                  <Select
                    ...
                    onChange={(option) => {
                      ...
                      setFormData(prev => ({
                        ...prev,
                        restaurantId: option ? option.value : ''
                      }));
                    ...
                    styles={{
                      control: (base, state) => ({
                        ...base,
                        borderColor: formErrors.restaurantId ? '#dc3545' : state.isFocused ? primaryColor : '#ced4da',
                        boxShadow: state.isFocused ? `0 0 0 0.2rem ${primaryColor}40` : 'none',
                        '&:hover': {
                          borderColor: formErrors.restaurantId ? '#dc3545' : primaryColor
                        }
                      })
                    }}
                  />
