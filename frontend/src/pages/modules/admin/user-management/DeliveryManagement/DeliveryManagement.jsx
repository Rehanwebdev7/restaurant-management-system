...
const DeliveryManagement = () => {
  ...
  const fetchRestaurants = async () => {
    ...
    try {
      const result = await ApiGet('/api/admin/users/filter', { role: 'restaurant', pageNumber: 0, pageSize: 1000 });
      if (result.success) setRestaurants(result.success.data.data.records || []);
    } catch (err) { } finally { setRestaurantsLoading(false); }
  };
  ...
  const exportToExcel = () => {
    const exportData = apiData.map(delivery => ({
      ...
      Branch: delivery.parentId?.name || 'N/A',
      Role: delivery.role || '',
    ...
    XLSX.writeFile(workbook, `delivery_${new Date().toISOString().split('T')[0]}.xlsx`);
  };

  const resetForm = () => {
    setFormData({ name: '', email: '', mobile: '', isActive: true, branchId: '' });
  ...
  const handleEdit = (user) => {
    ...
    const branchObj = branches.find(b => b.id === user.parentId?.id);
    setSelectedRestaurantId(branchObj?.parentId?.id ? String(branchObj.parentId.id) : '');
    setFormData({
      name: user.name || '',
      email: user.email || '',
      mobile: user.mobile || '',
      isActive: user.isActive ?? true,
      branchId: user.parentId?.id || ''
    });
    setFormErrors({});
    setModalMode('edit');
    setShowModal(true);
  ...
  const handleDelete = async (user) => {
    if (window.confirm(`Are you sure you want to delete delivery staff "${user.name}"?`)) {
      try {
        ...
        toast.error('Failed to delete delivery staff');
      }
    }
  };
...
// if (formData.password.trim()) {//   payload.password = formData.password.trim();// }
      ...
      toast.error('Failed to save delivery staff');
    } finally {
  ...
  const filteredBranches = selectedRestaurantId
    ? branches.filter(b => String(b.parentId?.id) === String(selectedRestaurantId))
    : [];
            ...
            {loading ? (
              ...
              paginatedData.map((delivery) => (
                <tr key={delivery.id}>
                  ...
                  <td>{delivery.parentId?.name || 'N/A'}</td>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} centered size="lg">
        <Modal.Header closeButton>
          <Modal.Title>
            ...
            {modalMode === 'add' ? 'Add Delivery Staff' : 'Edit Delivery Staff'}
          </Modal.Title>
        </Modal.Header>
              ...
              <Col md={6}>
                <Form.Group className="mb-3">
                  ...
                  <Select
                    ...
                    value={selectedRestaurantId ? { value: selectedRestaurantId, label: restaurants.find(r => String(r.id) === String(selectedRestaurantId))?.name } : null}
                    onChange={(selected) => {
              ...
              <Col md={6}>
                <Form.Group className="mb-3">
                  ...
                  <Select
                    ...
                    noOptionsMessage={() => selectedRestaurantId ? "No branches for this restaurant" : "Select restaurant first"}
                    styles={{
                      control: (base) => ({
                        ...base,
                  ...
                  />
                  {formErrors.branchId && (
                    <div className="text-danger" style={{ fontSize: '0.875em', marginTop: '0.25rem' }}>{formErrors.branchId}</div>
                  )}
                </Form.Group>
              </Col>
            </Row>
            ...
            >
              {formLoading ? (
                ...
                <>
                  ...
                  {modalMode === 'add' ? 'Add Delivery Staff' : 'Update Delivery Staff'}
                </>
              )}
            </Button>
          </Modal.Footer>
