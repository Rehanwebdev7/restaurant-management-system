...
const Delivered = () => {
  ...
  const handleExportExcel = () => {
    ...
    XLSX.writeFile(workbook, `DeliveredOrders_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
      ...
      <div className="user-table-container">
        <Row className="mb-3 g-2 align-items-center">
          <Col lg={3} md={4} sm={12}>
            <h4 className="mb-0" style={{ color: '#1e293b', fontWeight: '700', fontSize: '1.5rem' }}>
              <Badge bg="success" className="me-2">{totalRecords}</Badge>
              Delivered Orders
            </h4>
          </Col>
        ...
        <div className="table-responsive" style={{ background: '#f0f2f5', borderRadius: '12px' }}>
          <Table bordered hover className="modern-table mb-0">
            <thead style={{ background: 'linear-gradient(135deg, #198754 0%, #157347 100%)', color: 'white' }}>
              <tr>
                ...
                orders.map((order) => (
                  <tr key={order.id}>
                    ...
                    <td>
                      ...
                      {order.paymentMethod && <div className="text-muted small">{order.paymentMethod}</div>}
                    </td>
                    <td className="small">{formatDate(order.completedAt || order.createdAt)}</td>
                  </tr>
                ))
              ) : (
                <tr><td colSpan={9} className="text-center py-4">
                  <i className="bi bi-check2-circle fs-1 text-muted d-block mb-2"></i>
                  <span className="text-muted">No delivered orders found</span>
                </td></tr>
              )}
            </tbody>
          </Table>
        ...
        <Row className="align-items-center mt-4 pt-3" style={{ borderTop: '1px solid #f0f0f0' }}>
          ...
          <Col lg={5} md={4} sm={12} className="mb-2 mb-md-0 text-center">
            <span style={{ fontSize: '0.9rem', color: '#64748b', fontWeight: '500' }}>
              Showing <strong style={{ color: '#198754' }}>{totalRecords > 0 ? (currentPage - 1) * rowsPerPage + 1 : 0}</strong> to{' '}
              <strong style={{ color: '#198754' }}>{Math.min(currentPage * rowsPerPage, totalRecords)}</strong> of{' '}
              <strong style={{ color: '#198754' }}>{totalRecords}</strong> entries
            </span>
          </Col>
