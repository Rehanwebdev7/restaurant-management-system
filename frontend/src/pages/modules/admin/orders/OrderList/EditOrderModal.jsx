...
const EditOrderModal = ({ show, onClose, order }) => {
  ...
  const fetchMenuItems = async () => {
    ...
    try {
      const response = await ApiGet('/api/admin/menu_items/filter', {
        pageSize: 500,
        pageNumber: 0
      });
      if (response.success) {
  ...
  const fetchAddons = async () => {
    ...
    try {
      const response = await ApiGet('/api/admin/addons_items/getAll', {
        pageSize: 500,
        pageNumber: 0
      });
      if (response.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await ApiPut('/api/admin/orders/orderUpdate', payload);
