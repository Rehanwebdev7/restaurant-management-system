...
const Onway = () => {
  ...
  const fetchBranches = async () => {
    ...
    try {
      const response = await ApiGet('/api/admin/users/filter', { role: 'branch', pageSize: 100, pageNumber: 0 });
      if (response.success) {
  ...
  const handleExportExcel = () => {
    const exportData = orders.map(order => ({
      ...
      'Delivery Person': order.deliveryId?.name || '-',
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'On The Way Orders');
    XLSX.writeFile(workbook, `OnTheWayOrders_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
    ...
    <div className="onway-orders-page">
      <div className="user-table-container">
        ...
        <Row className="mb-2 g-2">
          <Col md={3} sm={6}>
            <Select options={restaurants.map(r => ({ value: r.id, label: r.name }))} value={selectedRestaurant} onChange={(selected) => { setSelectedRestaurant(selected); setSelectedBranch(null); setCurrentPage(1); }} isClearable isSearchable isLoading={restaurantsLoading} placeholder="All Restaurants" menuPortalTarget={document.body} menuPosition="fixed" styles={{ menuPortal: (base) => ({ ...base, zIndex: 9999 }) }} />
          </Col>
          <Col md={3} sm={6}>
            <Select options={filteredBranches.map(b => ({ value: b.id, label: b.name }))} value={selectedBranch} onChange={(selected) => { setSelectedBranch(selected); setCurrentPage(1); }} isClearable isLoading={branchesLoading} isDisabled={!selectedRestaurant} placeholder={selectedRestaurant ? "All Branches" : "Select Restaurant first"} menuPortalTarget={document.body} menuPosition="fixed" styles={{ menuPortal: (base) => ({ ...base, zIndex: 9999 }) }} />
          </Col>
        </Row>

        <Row className="mb-3 g-2">
          <Col lg={2} md={3} sm={6}>
            <Form.Control type="date" value={dateRange.fromDate} onChange={(e) => setDateRange(prev => ({ ...prev, fromDate: e.target.value }))} />
          </Col>
          <Col lg={2} md={3} sm={6}>
            <Form.Control type="date" value={dateRange.toDate} onChange={(e) => setDateRange(prev => ({ ...prev, toDate: e.target.value }))} />
          </Col>
          <Col lg={1} md={2} sm={6}>
            <Button variant="primary" onClick={handleDateFilter} className="w-100"><i className="bi bi-funnel me-1"></i>Filter</Button>
          </Col>
          <Col lg={4} md={6} sm={12}>
            <InputGroup>
        ...
        <div className="table-responsive" style={{ background: '#f0f2f5', borderRadius: '12px' }}>
          <Table bordered hover className="modern-table mb-0">
            <thead style={{ background: 'linear-gradient(135deg, #0dcaf0 0%, #0aa2c0 100%)', color: 'white' }}>
              <tr>
            ...
            <tbody>
              {loading ? (
                  ...
                  <tr key={order.id}>
                    <td>
                      <div className="d-flex gap-1 justify-content-center" style={{ whiteSpace: 'nowrap' }}>
                        {/* <Button size="sm" variant="outline-success" title="Mark Delivered" onClick={() => handleMarkDelivered(order)}>
                    ...
                    <td>
                      <div>{order.deliveryId?.name || '-'}</div>
                      <div className="text-muted small">{order.deliveryId?.mobile || '-'}</div>
                    </td>
                ...
                <tr><td colSpan={10} className="text-center py-4">
                  ...
                  <span className="text-muted">No orders on the way</span>
                </td></tr>
              )}
            </tbody>
          </Table>
        ...
        <Row className="align-items-center mt-4 pt-3" style={{ borderTop: '1px solid #f0f0f0' }}>
          ...
          <Col lg={5} md={4} sm={12} className="mb-2 mb-md-0 text-center">
            <span style={{ fontSize: '0.9rem', color: '#64748b', fontWeight: '500' }}>
              Showing <strong style={{ color: '#0dcaf0' }}>{totalRecords > 0 ? (currentPage - 1) * rowsPerPage + 1 : 0}</strong> to{' '}
              <strong style={{ color: '#0dcaf0' }}>{Math.min(currentPage * rowsPerPage, totalRecords)}</strong> of{' '}
              <strong style={{ color: '#0dcaf0' }}>{totalRecords}</strong> entries
            </span>
          </Col>
        ...
        <Modal show={showStatusModal} onHide={() => setShowStatusModal(false)} centered>
          <Modal.Header closeButton><Modal.Title>Mark as Delivered</Modal.Title></Modal.Header>
          <Modal.Body>
            Mark order <strong>{selectedOrder?.orderNumber}</strong> as <Badge bg="success">Delivered</Badge>?
          </Modal.Body>
          <Modal.Footer>
            <Button variant="secondary" onClick={() => setShowStatusModal(false)}>Cancel</Button>
            <Button variant="success" onClick={confirmStatusUpdate} disabled={statusUpdateLoading}>
              {statusUpdateLoading ? <Spinner size="sm" /> : 'Mark Delivered'}
            </Button>
          </Modal.Footer>
        </Modal>
      </div>
