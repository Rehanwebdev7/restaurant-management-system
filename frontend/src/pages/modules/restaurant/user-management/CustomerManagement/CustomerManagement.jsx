...
const CustomerManagement = () => {
  ...
  const fetchCustomerData = async () => {
    ...
    try {
      ...
      const result = await ApiGet('/api/restaurant/customers/getAll', params);
...
// if (formData.password.trim()) {//   dataToSend.password = formData.password.trim();// }
      ...
      let result;
      if (modalMode === 'add') {
        result = await ApiPost('/api/restaurant/customers/add', dataToSend);
      } else {
        result = await ApiPut('/api/restaurant/customers/update', dataToSend);
      }
  ...
  const handleToggleCod = async (customer) => {
    ...
    try {
      const result = await ApiPut('/api/restaurant/customers/update', {
        id: customer.id,
  ...
  const handleDelete = async (customer) => {
    ...
    try {
      const result = await ApiDelete(`/api/restaurant/customers/${customer.id}`);
