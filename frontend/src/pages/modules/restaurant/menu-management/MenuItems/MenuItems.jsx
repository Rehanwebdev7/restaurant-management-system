...
const MenuItems = () => {
  ...
  const [newAddonItems, setNewAddonItems] = useState([]);
  ...
  const fetchSubcategories = async () => {
    ...
    try {
      const result = await ApiGet('/api/restaurant/menu_subcategory/filter', {
        pageNumber: 0,
        pageSize: 1000
      });
  ...
  const fetchAddons = async () => {
    ...
    try {
      const result = await ApiGet('/api/restaurant/addons/filter', {
        pageNumber: 0,
        pageSize: 1000
      });
  ...
  const fetchMenuItemsData = async () => {
    ...
    try {
      ...
      const result = await ApiGet('/api/restaurant/menu_items/filter', params);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const result = await ApiPostFormData('/api/restaurant/menu_items/update_Menu', formDataToSend);
  ...
  const handleModalSubmit = async (e) => {
    ...
    try {
      ...
      const endpoint = modalMode === 'add'
        ? '/api/restaurant/menu_items/add_Menu'
        : '/api/restaurant/menu_items/update_Menu';
  ...
  const removeNewAddonItem = (index) => {
    setNewAddonItems(prev => prev.filter((_, i) => i !== index));
  };

  const updateNewAddonItem = (index, field, value) => {
    setNewAddonItems(prev => {
      const updated = [...prev];
  ...
  const validateAddonForm = () => {
    ...
    newAddonItems.forEach((item, index) => {
      if (!item.name.trim()) {
  ...
  const handleAddonModalSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        branchId: { id: parseInt(addonFormData.branchId) },
        ...
        addonItems: newAddonItems.map(item => ({
          name: item.name.trim(),
      ...
      const result = await ApiPost('/api/restaurant/addons/add_addonItem', payload);
                  ...
                  <Col md={6}>
                    <Form.Group className="mb-3">
                      ...
                      <Select
                        ...
                        value={addonFormData.branchId ? {
                          value: Number(addonFormData.branchId),
                          label: branches.find(b => b.id === Number(addonFormData.branchId))?.name || `Branch ${addonFormData.branchId}`
                        } : null}
                        onChange={(selected) => {
                          setAddonFormData(prev => ({ ...prev, branchId: selected ? selected.value : '' }));
                          if (addonFormErrors.branchId) {
                            setAddonFormErrors(prev => ({ ...prev, branchId: '' }));
                          }
                        }}
                        isClearable
                        isSearchable
                        ...
                        styles={{
                          control: (base) => ({
                            ...base,
                            borderColor: addonFormErrors.branchId ? '#dc3545' : base.borderColor
                          })
                        }}
                      />
                      {addonFormErrors.branchId && (
                        <div className="text-danger" style={{ fontSize: '0.875em', marginTop: '0.25rem' }}>
                          {addonFormErrors.branchId}
                        </div>
                      )}
                        ...
                        <tr key={index}>
                          <td>
                            <Form.Control
                              ...
                              size="sm"
                              value={item.name}
                              onChange={(e) => updateNewAddonItem(index, 'name', e.target.value)}
                          ...
                          <td>
                            <Form.Control
                              ...
                              step="0.01"
                              value={item.price}
                              onChange={(e) => updateNewAddonItem(index, 'price', e.target.value)}
                              min="0"
                          ...
                          <td>
                            <Form.Select
                              ...
                              onChange={(e) => updateNewAddonItem(index, 'attribute', e.target.value)}
                            >
                          ...
                          <td className="text-center">
                            <Form.Check
                              ...
                              onChange={(e) => updateNewAddonItem(index, 'isActive', e.target.checked)}
                            />
                          </td>
                          <td className="text-center">
                            <Button
                              variant="outline-danger"
                              size="sm"
                              onClick={() => removeNewAddonItem(index)}
                            >
                              <i className="bi bi-trash"></i>
                            </Button>
                          </td>
