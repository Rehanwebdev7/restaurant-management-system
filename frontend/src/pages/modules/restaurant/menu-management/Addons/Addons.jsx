...
const Addons = () => {
  ...
  const fetchAddonsData = async () => {
    ...
    try {
      ...
      if (statusFilter) {
        params.isActive = statusFilter === 'active' ? 1 : 0;
      }

      const result = await ApiGet('/api/restaurant/addons/filter', params);
  ...
  const fetchAddonItems = async (addonsId) => {
    try {
      const result = await ApiGet('/api/restaurant/addons_items/addonsId', { id: addonsId });
  ...
  // View addon items
  const handleViewItems = async (addon) => {
    ...
    try {
      const result = await ApiGet(`/api/restaurant/addons_items/addonId?addonId=${addon.id}`);
      if (result.success) {
        const data = result.success.data?.data || result.success.data || [];
        // Handle both array and object with addonItems property
        const items = Array.isArray(data) ? data : (data.addonItems || data.records || []);
        setViewItems(items);
      } else {
        toast.error('Failed to fetch addon items');
      }
  ...
  const handleEdit = async (addon) => {
    ...
    setSelectedBranchName(addon.branchId?.name || '');
    setFormErrors({});
    setModalMode('edit');
    setShowModal(true);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const result = await ApiPut('/api/restaurant/addons/update_addonItem', payload);
    ...
    try {
      ...
      if (modalMode === 'add') {
        ...
        if (formData.branchId) {
          payload.branchId = { id: parseInt(formData.branchId) };
        }

        result = await ApiPost('/api/restaurant/addons/add_addonItem', payload);
      } else {
        // Update payload - includes id and branchId
        payload = {
          id: selectedAddon.id,
          name: formData.name.trim(),
          description: formData.description.trim(),
        ...
        result = await ApiPut('/api/restaurant/addons/update_addonItem', payload);
      }
            ...
            {loading ? (
              ...
              paginatedData.map((addon) => (
                <tr key={addon.id}>
                  ...
                  {/* <td>{addon.restaurantId?.name || 'N/A'}</td> */}
                  <td>
              ...
              <tbody>
                {viewItems.map((item, index) => (
                  <tr key={item.id || index}>
                    <td>{item.name || 'N/A'}</td>
                    <td>${item.price || 0}</td>
                    <td><Badge bg="secondary">{item.attribute || 'N/A'}</Badge></td>
                    <td>
                      {item.isActive ? (
