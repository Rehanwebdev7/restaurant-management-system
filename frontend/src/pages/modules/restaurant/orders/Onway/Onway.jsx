...
const Onway = () => {
  ...
  useEffect(() => {
    fetchOrders();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentPage, rowsPerPage, selectedBranch]);
  ...
  const fetchBranches = async () => {
    ...
    try {
      const response = await ApiGet('/api/restaurant/users/filter', { role: 'branch', pageSize: 100, pageNumber: 0 });
      if (response.success) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await ApiDelete(`/api/restaurant/orders/${selectedOrder.id}`);
      if (response.success) {
  ...
  const confirmStatusUpdate = async () => {
    ...
    try {
      const response = await ApiPut('/api/restaurant/orders/update', {
        id: selectedOrder.id,
        status: 'DELIVERED'
      });
    ...
    <div className="onway-orders-page">
      <div className="user-table-container">
        ...
        <Row className="mb-3 g-2">
          <Col lg={3} md={4} sm={6}>
            <Select options={branches} value={selectedBranch} onChange={setSelectedBranch} isClearable isLoading={branchesLoading} placeholder="Select Branch" />
          </Col>
          <Col lg={2} md={3} sm={6}>
