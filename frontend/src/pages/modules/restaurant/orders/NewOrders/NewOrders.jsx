...
const NewOrders = () => {
  ...
  const paymentStatusBadges = {
    ...
    'SUCCESS': { bg: 'success', label: 'Paid' },
    'COMPLETED': { bg: 'success', label: 'Paid' },
  ...
  // Quick status update (Accept & Prepare / Mark Ready)
  const handleQuickStatusUpdate = async (order, newStatus, successMessage) => {
    try {
      const response = await ApiPut('/api/restaurant/orders/update', {
        id: order.id,
        status: newStatus
            ...
            <tbody>
              {loading ? (
                ...
                orders.map((order) => (
                  <tr key={order.id}>
                    <td>
                      <div className="d-flex gap-1 justify-content-center flex-wrap" style={{ whiteSpace: 'nowrap' }}>
                        <Button size="sm" variant="success" title="Accept & Prepare" onClick={() => handleQuickStatusUpdate(order, 'PREPARING', `Order #${order.orderNumber} moved to Preparing`)}>
                    ...
                    <td>
                      ...
                      {order.paymentStatus === 'PENDING' && (
                        <Button size="sm" variant="outline-success" className="mt-1" style={{ fontSize: '0.7rem', padding: '2px 6px' }}
