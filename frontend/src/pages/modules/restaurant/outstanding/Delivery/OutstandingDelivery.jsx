...
const OutstandingDelivery = () => {
  ...
  const fetchDeliveryData = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Filter only users with outstanding balance > 0
        const filteredRecords = (data.records || []).filter(u => (u.outstandingBalance ?? 0) > 0);
        setApiData(filteredRecords);
  ...
  const handleClearSubmit = async (e) => {
    ...
    try {
      ...
      const result = await ApiPost('/api/restaurant/outstanding/deduct', payload);
