...
const CardDeposit = () => {
  ...
  const filteredData = useMemo(() => {
    return deposits.filter(deposit => {
      const matchesSearch =
        ...
        deposit.customerMobile.includes(searchQuery) ||
        deposit.customerEmail.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  const handleActionClick = (deposit) => {
    navigate(`/restaurant/recurring-deposit/card-deposit/${deposit.id}`, {
      state: { deposit }
    });
  };
      ...
      <div className="user-table-container">
          ...
          <Table bordered hover className="modern-table" style={{ background: '#f0f2f5' }}>
            ...
            <tbody>
              {currentData.length > 0 ? (
                currentData.map((deposit) => (
                  <tr key={deposit.id} style={{ background: '#f0f2f5' }}>
                    <td>
                      <Button
                        size="sm"
                    ...
                    <td>{new Date(deposit.startDate).toLocaleDateString('en-IN')}</td>
                    <td>
                      <strong>{deposit.customerName}</strong>
                    </td>
                    <td>
                      {deposit.customerMobile}
                    </td>
                    <td>
                      {deposit.customerEmail}
                    </td>
                    <td>
                      <span className={`badge bg-${deposit.status === 'active' ? 'success' : deposit.status === 'completed' ? 'primary' : 'danger'}`}>
                        {deposit.status.charAt(0).toUpperCase() + deposit.status.slice(1)}
                      </span>
                    </td>
                    <td>
                      <div>
                        <strong>${deposit.monthlyAmount.toLocaleString('en-IN')}/month</strong>
                        <br />
                        <small className="text-muted">Total: ${deposit.totalAmount.toLocaleString('en-IN')}</small>
                      </div>
                    </td>
                  </tr>
                ))
        ...
        <Modal show={showModal} onHide={() => setShowModal(false)} centered>
          <Modal.Header closeButton>
            <Modal.Title>Add New Deposit Card</Modal.Title>
          </Modal.Header>
