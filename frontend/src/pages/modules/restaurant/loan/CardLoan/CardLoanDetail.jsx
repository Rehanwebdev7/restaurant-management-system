...
const CardLoanDetail = () => {
  const location = useLocation();
  const navigate = useNavigate();
  const { id } = useParams();
  const loanData = location.state?.loan;
  ...
  const amounts = useMemo(() => {
    ...
    const total = loanData?.totalAmount || (loanData?.monthlyAmount * loanData?.duration) || 60000;
    ...
    return (
      <Container fluid className="mt-5 text-center">
        <div className="py-5">
          ...
          <h4 className="text-muted mb-4">Loan data not found. Please go back and try again.</h4>
          ...
          <Col lg={4} md={4} sm={12} className="d-flex align-items-center gap-2">
            ...
            <div>
              ...
              <small className="text-muted">
                {loanData.customerName} - {loanData.customerMobile}
              </small>
            </div>
          </Col>
