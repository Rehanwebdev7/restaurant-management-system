...
const BranchSidebar = ({ collapsed, visible, onLogout }) => {
  ...
  useEffect(() => {
    const savedFavorites = localStorage.getItem('branchSidebarFavorites');
    if (savedFavorites) {
      try {
  ...
  const saveFavorites = (newFavorites) => {
    localStorage.setItem('branchSidebarFavorites', JSON.stringify(newFavorites));
  ...
  // Define branch navigation items
  const allNavItems = [
    {
    ...
    {
      path: '/branch/outstanding',
      icon: 'bi bi-wallet2',
      label: 'Outstanding',
      submenu: [
        { path: '/branch/outstanding/delivery', label: 'Delivery' },
        { path: '/branch/outstanding/history', label: 'Outstanding History' }
      ]
    },
    {
      path: '/branch/orders',
      ...
      label: 'Orders',
      submenu: [
        { path: '/branch/orders/list', label: 'All Orders' },
        { path: '/branch/orders/new', label: 'New Orders' },
        { path: '/branch/orders/preparing', label: 'Preparing' },
        { path: '/branch/orders/onway', label: 'On The Way' },
        { path: '/branch/orders/delivered', label: 'Delivered' },
        { path: '/branch/orders/cancelled', label: 'Cancelled' }
      ]
    },
    {
      path: '/branch/wallet-topup',
      icon: 'bi bi-wallet-fill',
      label: 'Withdrawal Wallet',
      submenu: [
        { path: '/branch/wallet-topup/requests', label: 'Withdrawal Request' },
        { path: '/branch/wallet-topup/history', label: 'Withdrawal History' }
      ]
    },
    {
      ...
      label: 'Settings',
      submenu: [
        { path: '/branch/settings/bank-details', label: 'Bank Details' }
      ]
    },
    {
      path: '/branch/menu-management',
    ...
    },
    {
      ...
      submenu: [
        { path: '/branch/user-management/cashier', label: 'Cashier' },
        { path: '/branch/user-management/kitchen', label: 'Kitchen' },
        { path: '/branch/user-management/delivery', label: 'Delivery' },
        { path: '/branch/user-management/customers', label: 'Customers' }
      ]
    }
  ];

  // Branch navigation items (no filtering needed)
  ...
  return (
    <div className={`dms-sidebar ${collapsed ? 'collapsed' : ''} ${visible ? 'show' : ''}`}>
      ...
      {/* User Profile Card */}
      <div className="sidebar-user-card" title={collapsed ? `${userName || 'User'} - ${userRole || 'Branch'}` : ''}>
        ...
        {!collapsed && (
          <div className="sidebar-user-info">
            ...
            <span className="sidebar-user-role">{(userRole || 'Branch').replace(/_/g, ' ')}</span>
          </div>
        )}
      </div>
