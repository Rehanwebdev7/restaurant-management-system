...
const KitchenSidebar = ({ collapsed, visible, onLogout }) => {
  ...
  useEffect(() => {
    const savedFavorites = localStorage.getItem('kitchenSidebarFavorites');
    if (savedFavorites) {
      try {
  ...
  const saveFavorites = (newFavorites) => {
    localStorage.setItem('kitchenSidebarFavorites', JSON.stringify(newFavorites));
  ...
  return (
    <div className={`dms-sidebar ${collapsed ? 'collapsed' : ''} ${visible ? 'show' : ''}`}>
      ...
      {/* User Profile Card */}
      <div className="sidebar-user-card" title={collapsed ? `${userName || 'User'} - ${userRole || 'Kitchen'}` : ''}>
        ...
        {!collapsed && (
          <div className="sidebar-user-info">
            ...
            <span className="sidebar-user-role">{(userRole || 'Kitchen').replace(/_/g, ' ')}</span>
          </div>
        )}
      </div>
